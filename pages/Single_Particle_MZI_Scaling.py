...
with st.sidebar:
    ...
    omega_true = st.number_input(
        r"True $\omega$ (rad/unit time)",
        value=1.0,
        min_value=0.01,
        max_value=10.0,
        step=0.01,
...
with st.spinner("Running validation checks..."):
    val = run_validation(omega=omega_true, t_hold=1.0)
...
if run_btn:
    with st.spinner("Running sensitivity sweep..."):
        ...
        result_analytical = fit_scaling_exponent(
            df["t_hold"].to_numpy(),
            df["delta_omega_analytical"].to_numpy(),
        )
        result_numerical = fit_scaling_exponent(
            df["t_hold"].to_numpy(),
            df["delta_omega_numerical"].to_numpy(),
        )
    ...
    st.subheader(r"∂⟨J_z⟩/∂ω: Analytical vs Numerical")
    ...
    with st.expander("📊 View Raw Data", expanded=False):
        display_df = df.copy()
        display_df["t_hold"] = display_df["t_hold"].map("{:.4f}".format)
        ...
        display_df["delta_omega_analytical"] = display_df["delta_omega_analytical"].map(
            "{:.6e}".format,
        )
        display_df["delta_omega_numerical"] = display_df["delta_omega_numerical"].map(
            "{:.6e}".format,
        )
        display_df["delta_omega_theory"] = display_df["delta_omega_theory"].map(
            "{:.6e}".format,
        )
