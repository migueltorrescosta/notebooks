...
def compute_estimation_df(
    ...
    for d_val in np.round(
        np.linspace(guessed_delta_s - delta_s_var, guessed_delta_s + delta_s_var, 51),
        3,
    ):
...
def calculate_likelihood(
    prob: float,
    true_pdf: np.ndarray | None = None,
) -> float:
    if true_pdf is None:
        true_pdf = calculate_probability_density_function(
            n=n_trials,
            p=true_probability,
        )
...
estimation_df["likelihood"] = np.divide(
    estimation_df["likelihood"],
    estimation_df["likelihood"].mean(),
)
...
if st.sidebar.toggle("Show history", value=False):
    ...
    with st.sidebar:
        ...
        history_y_axis = st.selectbox("y-axis", sorted(history_data.keys()))
        history_x_axis = st.selectbox("x-axis", sorted(history_data.keys()))
    ...
    if "experiment_history_df" not in st.session_state:
        ...
        st.session_state.experiment_history_df = (
            st.session_state.experiment_history_df.reset_index(drop=True)
        )
        st.session_state.experiment_history_df.loc[
            len(st.session_state.experiment_history_df)
        ] = history_data
        st.session_state.experiment_history_df = (
            st.session_state.experiment_history_df.drop_duplicates()
        )
