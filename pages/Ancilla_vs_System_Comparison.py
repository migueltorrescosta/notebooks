...
with st.sidebar:
    ...
    T_H = st.slider(
        "Holding-time strength T_H",
        min_value=0.1,
        max_value=5.0,
        value=1.0,
        step=0.1,
        help="Controls how strongly the phase and interaction act. QFI scales as T_H².",
    )
...
with col1:
    st.metric(
        ...
        f"F_Q = {analytical_fq_B_max(T_H):.0f}",
        f"Δθ = 1/√{analytical_fq_B_max(T_H):.0f}",
    )

with col2:
    st.metric(
        "Case A (1 particle + ancilla, α = 0)",
        f"F_Q = {analytical_fq_A_zero(T_H):.0f}",
        "Δθ = 1",
    )

with col3:
    ratio_theory = np.sqrt(analytical_fq_B_max(T_H) / analytical_fq_A_zero(T_H))
    st.metric(
        "Ratio ℛ = Δθ_A / Δθ_B",
        f"{ratio_theory:.2f}",
...
if run_button:
    ...
    with mcol2:
        st.metric(
            "Case B F_Q (theory)",
            f"{analytical_fq_B_max(T_H):.4f}",
            delta=f"{(result.fq_B_max / analytical_fq_B_max(T_H) - 1) * 100:.2f}%",
        )
    with mcol3:
        st.metric(
    ...
    with st.expander("Detailed Results", expanded=False):
        ...
        if result.fq_A_theta:
            st.markdown("**θ-dependence of QFI (Case A):**")
            theta_df = {
                "θ": list(result.fq_A_theta.keys()),
                "F_Q": [f"{v:.6f}" for v in result.fq_A_theta.values()],
            }
            st.table(theta_df)
