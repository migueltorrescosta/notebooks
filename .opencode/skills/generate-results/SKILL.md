...
2. Explore existing implemented code in `src/` **and** the report's own experiment module (for example, `phase_modulated_drive.py`) to understand available functions. Functions for the report's simulation might live in either location.
...
1. **Read the report**: Open the target report in `reports/` and identify:
   ...
   - The expected output quantities (sensitivity versus N, quantum Fisher information (QFI) values, and so on)
...
- If minor fixes are needed (for example, missing parameter in `to_dataframe()`, incorrect default), make the minimal change.
...
- Plot the standard quantum limit (SQL) and Heisenberg limit (HL) as reference lines (dashed) when showing scaling plots.
