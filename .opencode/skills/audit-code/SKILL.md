...
5. Prioritise violations by severity: correctness > deduplication (shared-infrastructure promotion) > performance > style.
...
2. **Read the relevant code**. Open all source files related to this report:
   - The report's own experiment module (for example, `phase_modulated_drive.py`) and its test file (`test_*.py`) in `reports/rYYYYMMDD/`
...
3. **Standard violated**: Which standard (for example, "Code Architecture §Serialization completeness")
