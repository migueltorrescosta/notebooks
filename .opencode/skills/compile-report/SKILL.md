...
1. For each CSV or Parquet file in `reports/{date}/raw_data/`:
   - Load it using the corresponding dataclass's `from_csv()` or `from_parquet()` method
...
2. For each SVG in `reports/{date}/figures/`:
