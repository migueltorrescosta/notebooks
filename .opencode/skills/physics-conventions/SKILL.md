...
- **φ** (phi): MZI phase shift (the parameter being estimated by the interferometer). `phi_bs` is BS reflection phase; `phi_Bloch` is Bloch-sphere azimuth. Measurement weight renamed to `psi_opt`.
...
- **Δω**: Sensitivity (standard deviation of estimate) by error propagation. SQL comparison: `Δω_SQL = 1/t_hold`.
