...
_core = importlib.import_module("reports.r20260701.partial_omega_modulated_drive")
...
def _merge_random_search_parquets() -> None:
    ...
    """
    raw_dir = REPORTS_DIR / f"r{PARTIAL_DATE}" / "raw_data"
...
def run_cli() -> None:
    ...
    (REPORTS_DIR / f"r{PARTIAL_DATE}" / "raw_data").mkdir(parents=True, exist_ok=True)
    (REPORTS_DIR / f"r{PARTIAL_DATE}" / "figures").mkdir(parents=True, exist_ok=True)
