...
- **This report**: Keeps the ancilla and measures the **optimised** linear combination $M = m_s J_z^S + m_a J_z^A$. Both the coupling $\alpha_{xx}$ and the measurement coefficients $(m_s, m_a)$ are jointly optimised for each $(\omega, N)$ pair. The fair sensitivity benchmark is the $2N$-particle SQL $\Delta\omega_{\text{SQL}} = 1/(\sqrt{2N}\,T_H)$, because measurements access both subsystems.
...
2. **Beam splitter on both subsystems**: A 50/50 symmetric beam splitter acts independently on each subsystem, generated by $J_x$ with angle $\pi/2$:
   ...
   Both single-subsystem BS unitaries are $(N+1) \times (N+1)$ matrix exponentials computed by `scipy.linalg.expm`, and the combined unitary is their Kronecker product.
...
3. **Holding period with simultaneous phase encoding and XX interaction**: The full state evolves under the total Hamiltonian $H = H_S + H_A + H_{\text{int}}$ for duration $T_H = 10$. The three terms are:
   ...
   The hold unitary is $U_{\text{hold}}(T_H) = \exp(-i T_H H)$, computed by `scipy.linalg.expm`. The matrix dimension is $(N+1)^2 \times (N+1)^2$, ranging from $4\times4$ ($N=1$) to $441\times441$ ($N=20$).
...
5. **Construct the measurement operator**: The joint observable is
   ...
   parameterised by $\psi \in [-\pi, \pi]$. The coefficients automatically satisfy $m_s^2 + m_a^2 = 1$ with $m_s = \cos\psi$, $m_a = \sin\psi$. The measurement operator $M$ is an $(N+1)^2 \times (N+1)^2$ Hermitian matrix in the full space.
...
6. **Measure $M$ on the full final state**: The expectation value and variance are computed directly from the pure final state vector $|\Psi_{\text{final}}\rangle$:
   ...
   No partial trace is performed, so all information from both S and A is retained.
...
Because $m_s^2 + m_a^2 = 1$, the variance is independent of $\psi$ at $\alpha_{xx}=0$: it equals the single-MZI variance $N/4$ for a coherent spin state (CSS). The derivative is:
...
1. **Operator construction** — Build $J_z$, $J_x$, $J_y$ as $(N+1) \times (N+1)$ Dicke-basis matrices using existing `dicke_basis.jz_operator(N)`, etc. Embed into the combined space by Kronecker products, as in 20260522. Additionally, build the identity $\mathbb{1}_{(N+1)^2}$ for full-space operations.
...
5. **Full-state measurement** — Construct $M(\psi) = \cos\psi \, J_z^S + \sin\psi \, J_z^A$ as a full-space operator. Compute $\langle M \rangle = \langle\Psi| M |\Psi\rangle$ and $\langle M^2 \rangle = \langle\Psi| M^2 |\Psi\rangle$ directly from the final state vector $|\Psi\rangle$, without any partial trace. The variance is $\text{Var}(M) = \langle M^2 \rangle - \langle M \rangle^2$, clamped to zero when below $10^{-12}$ because of numerical round-off.

6. **Sensitivity computation** — Compute $\Delta\omega(\alpha_{xx}, \psi) = \sqrt{\text{Var}(M)} / |\partial\langle M\rangle/\partial\omega|$ by central finite differences with $\delta = 10^{-6}$. The full circuit (including all operators) is re-evaluated at $\omega \pm \delta$.

7. **2D optimisation over $(\alpha_{xx}, \psi)$** — For each $(\omega, N)$ pair, minimise $\Delta\omega(\alpha_{xx}, \psi)$ using L-BFGS-B (bounded) with 20 random starting points. The optimisation variables are:
   - $\alpha_{xx} \in [0, 20]$ (bounded),
   - $\psi \in [-\pi, \pi]$ (bounded, periodic).
   
   **Procedure for each $(\omega, N)$**:
   - Generate 20 random seeds: $\alpha_{xx}^{(0)} \sim U[0, 20]$, $\psi^{(0)} \sim U[-\pi, \pi]$.
   - For each seed, run L-BFGS-B (using `scipy.optimize.minimize`) with numerical gradient (finite differences, step $10^{-6}$ in each parameter).
   - Record the best $(\alpha_{xx}^*, \psi^*)$ across all 20 starts, along with the achieved $\Delta\omega_{\text{opt}}$.
   - **Convergence diagnostic**: Flag any $(\omega, N)$ where the best result is found by only one start (possible local minimum issue).
...
**Computational cost estimate**: $50 \, \omega \times 20 \, N \times 20$ random starts $\times$ about 80 BFGS iterations $\times 3$ circuit evaluations (central difference) $= 4.8$ million circuit evaluations. For $N=20$ ($441\times441$ matrix exponentials at about 10 ms), this is about 13 hours for the largest $N$. Smaller $N$ values are much faster ($N=1$: $4\times4$, $\ll 1$ ms), giving a weighted total of roughly three to five hours. The sweep is parallelisable over $(\omega, N)$ pairs.
...
| Random starts | 20 for each $(\omega, N)$ pair | Avoid local minima in 2D landscape |
...
- **Sensitivity** — $\Delta\omega = \sqrt{\text{Var}(M)} / |\partial\langle M\rangle/\partial\omega|$ by central finite differences ($\delta = 10^{-6}$) — adapted from existing code.
- **$(\alpha_{xx}, \psi)$ optimisation** — L-BFGS-B with 20 random starts for each $(\omega, N)$ pair — implemented.
...
| **Optimiser failure — many local minima** — the 2D $(\alpha_{xx}, \psi)$ landscape has multiple local minima, and 20 random starts are insufficient to find the global optimum | Increase to 40 random starts for a subset of $(\omega, N)$ and compare results. If the best result is consistently found by only one or two starts, flag these points and increase the start count. Use the coarse 2D grid to map the landscape shape for representative points. |
...
| `20260525-scaling-exponents.svg` | Exponent $\alpha$ versus $\omega$ panel |
| `20260525-comparison-traced-out.svg` | $\Delta\omega_{\text{opt}}$ (this report, $2N$-SQL) versus 2026-05-22 traced-out ($N$-SQL) comparison |
...
*Figure 1: Decoupled baseline ($\alpha_{xx}=0$): $\Delta\omega_{\text{opt}}$ versus $\omega$ for selected $N$, compared against the $2N$-SQL. The optimal measurement at every point is $\psi^* = \pi/4$, and $\Delta\omega_{\text{opt}}$ exactly matches $\Delta\omega_{\text{SQL}}$.*
...
*Figure 5: N-scaling at $\omega = 0.3$: $\Delta\omega_{\text{opt}}$ versus $N$ on log-log axes, with $2N$-SQL and $N$-SQL reference lines. The data exactly overlay the $2N$-SQL line.*

![N-scaling at $\omega = 1.0$](figures/20260525-n-scaling-theta1.0.svg)
*Figure 6: N-scaling at $\omega = 1.0$, showing identical behaviour: $\Delta\omega_{\text{opt}}$ follows the $2N$-SQL exactly.*
...
*Figure 8: Scaling exponent $\alpha$ versus $\omega$ (top) and $R^2$ (bottom). Every $\omega$ gives $\alpha = -0.5$ with $R^2 = 1.0$, confirming SQL-limited scaling across the entire $\omega$ range.*
...
Because $\Delta\omega_{\text{opt}} = \Delta\omega_{\text{SQL}}$ at all points, the $\omega$-dependence is trivial: $\Delta\omega_{\text{opt}}$ is independent of $\omega$ and depends only on $N$ through $1/(\sqrt{2N} T_H)$. The optimal $\psi^* = \pi/4$ is also independent of $\omega$. No oscillatory structure from the XX coupling is observed, because the optimiser always selects $\alpha_{xx}=0$, eliminating any coupling-induced dynamics.
...
**Key Finding**: The joint measurement protocol outperforms the traced-out protocol by a factor of $\sqrt{2}$, purely from accessing the full system-ancilla state. However, this improvement is exactly the SQL improvement from doubling the particle number, so no genuine XX advantage is present.
...
- **$2N$-SQL violation through the XX coupling** — $\exists (\omega, N)$ such that $\Delta\omega_{\text{opt}} < 1/(\sqrt{2N} T_H)$ — **FAIL** ($r_{\text{XX}} = 1.0$ at all 500 points)
...
**QFI bound for the full pure state**: For the full pure state $|\Psi(\omega)\rangle$, the quantum Fisher information (QFI) is $F_Q = 4 (\langle G_{\text{eff}}^2 \rangle - \langle G_{\text{eff}} \rangle^2)$, where $G_{\text{eff}}$ is the effective generator $G_{\text{eff}} = -i (\partial U_{\text{total}}/\partial \omega) U_{\text{total}}^\dagger$. The error-propagation sensitivity with an optimised measurement satisfies $\Delta\omega \geq \Delta\omega_Q = 1/\sqrt{F_Q}$. If the optimised $M$ is chosen optimally, it might approach but not exceed the QFI bound: $\Delta\omega_{\text{opt}} \geq 1/\sqrt{F_Q}$.
...
If the XX coupling generates entanglement, the QFI can increase beyond $2N T_H^2$, enabling $\Delta\omega_Q < \Delta\omega_{\text{SQL}}$. Our error-propagation sensitivity with optimal $M$ might approach this bound if $M$ is chosen near the optimal observable, but the bound $r_{\text{XX}} \geq 1/\sqrt{F_Q / (2N T_H^2)}$ applies.
...
5. **Joint measurement versus traced-out** — the optimised joint measurement (this report) outperforms the traced-out protocol (2026-05-22) by a factor $\sqrt{2}$, purely from accessing the ancilla information. This improvement is exactly the SQL improvement from doubling the particle number, so no genuine XX advantage is present.

**Broader implications**: The XX coupling $J_x^S \otimes J_x^A$ appears to be genuinely inactive for metrology in the dual-MZI protocol, even with access to the full system-ancilla state and an optimised joint measurement. The commutator $[M, H_{\text{int}}] \neq 0$ is not sufficient to generate a metrological advantage: the XX interaction might entangle S and A, but it does so in a way that does not improve the signal-to-noise ratio for estimating $\omega$. This is consistent with the 2026-05-22 result, where tracing out the ancilla also showed no XX advantage (against the $N$-SQL). The inability to beat even the $2N$-SQL (which is a harder benchmark) confirms that the XX coupling fundamentally does not generate $J_z$-correlated entanglement useful for phase estimation in this geometry.

**Possible scenario**: The XX interaction might entangle S and A in the $J_x$ basis, but the measurement $M$ is in the $J_z$ basis. The entanglement that improves $J_z$ sensitivity might require a different interaction structure (for example, $J_z^S \otimes J_z^A$ Ising coupling or $J_z^S \otimes J_x^A$ anisotropic coupling).

**Open items**: (a) If the XX coupling alone remains inactive even with joint measurements, would adding the off-diagonal coupling terms ($H_y, H_{\text{diff}}$) from 2026-05-21 amplify the effect when combined with an optimised joint measurement? (b) Might the joint measurement approach activate the ZZ (Ising) interaction $H_{\text{int}} = \alpha_{zz} J_z^S \otimes J_z^A$, where the commutator $[M, H_{\text{int}}] = 0$ at $\psi = \pi/4$ but $[M, H_{\text{int}}] \neq 0$ for other $\psi$, potentially enabling higher-order metrological gain? (c) For a non-linear measurement (for example, parity, photon-number correlations) rather than a linear combination of $J_z$ operators, might the XX coupling be harnessed more effectively? (d) What is the effect of initial entanglement between S and A (rather than a product state) combined with the optimised joint measurement?
