...
- **`norm_ball_sampling()`** — A new function in `reports/20260527/drive_component_analysis.py` implementing Marsaglia's method for the 3-ball, driving `drive_sensitivity_objective` for each sample, and returning a structured array with all metadata.
...
Every term in $H$ commutes with $J_z^A$ because $[J_z^S, J_z^A] = 0$, $[J_z^A, J_z^A] = 0$, and $[J_z^S \otimes J_z^A, J_z^A] = 0$, so $[J_z^A, H] = 0$ and $J_z^A$ is a constant of motion. The Hilbert space splits into invariant $J_z^A$ eigenspaces; the ancilla begins in $|0\rangle_A$ satisfying $J_z^A|0\rangle_A = m_A |0\rangle_A$ with $m_A = 1/2$ and remains there throughout. Since all Hamiltonian terms also pairwise commute ($[\omega J_z^S, a_z J_z^A] = 0$, $[\omega J_z^S, a_{zz} J_z^S\otimes J_z^A] = 0$, $[a_z J_z^A, a_{zz} J_z^S\otimes J_z^A] = 0$), the hold propagator factorises as $U_H = e^{-iT_H H} = e^{-i a_z T_H J_z^A} e^{-iT_H(\omega J_z^S + a_{zz} J_z^S \otimes J_z^A)}$. Acting on the ancilla eigenstate gives $e^{-i a_z T_H J_z^A}|0\rangle_A = e^{-i a_z T_H/2}|0\rangle_A$, so the $a_z$ dependence reduces to an unobservable global phase. Within the $m_A = 1/2$ eigenspace, the operator $J_z^S \otimes J_z^A$ acts as $m_A J_z^S = J_z^S/2$; dropping the global phase, the effective system Hamiltonian is $H_{\rm eff} = \Omega J_z^S$ with $\Omega = \omega + a_{zz}/2$. The full four-dimensional problem thus reduces to a single spin-$1/2$ evolving under $\Omega J_z^S$ for time $T_H$, sandwiched between two beam splitters.

We compute the state explicitly through the circuit. The first BS acts as $U_{\rm BS}|0\rangle_S = (|0\rangle - i|1\rangle)/\sqrt{2}$, giving $|\Psi_1\rangle = (|0\rangle - i|1\rangle)|0\rangle_A/\sqrt{2}$. The effective hold evolution $U_{\rm eff} = e^{-iT_H \Omega J_z^S}$ sends $|0\rangle$ to $e^{-i\Omega T_H/2}|0\rangle$ and $|1\rangle$ to $e^{i\Omega T_H/2}|1\rangle$, so up to the global phase $e^{-i a_z T_H/2}$, $|\Psi_2\rangle = (e^{-i\Omega T_H/2}|0\rangle - i e^{i\Omega T_H/2}|1\rangle)|0\rangle_A/\sqrt{2}$. The second BS acts as $U_{\rm BS}|0\rangle = (|0\rangle - i|1\rangle)/\sqrt{2}$ and $U_{\rm BS}|1\rangle = (|1\rangle - i|0\rangle)/\sqrt{2}$, giving $|\Psi_3\rangle = (e^{-i\Omega T_H/2}(|0\rangle - i|1\rangle) - i e^{i\Omega T_H/2}(|1\rangle - i|0\rangle))|0\rangle_A/2 = [(e^{-i\Omega T_H/2} - e^{i\Omega T_H/2})|0\rangle - i(e^{-i\Omega T_H/2} + e^{i\Omega T_H/2})|1\rangle]|0\rangle_A/2 = [-i\sin(\Omega T_H/2)|0\rangle - i\cos(\Omega T_H/2)|1\rangle]|0\rangle_A = -i[\sin(\Omega T_H/2)|0\rangle + \cos(\Omega T_H/2)|1\rangle]|0\rangle_A$. Up to the global phase $-i$, the system state is $|\psi_S\rangle = \sin(\Omega T_H/2)|0\rangle + \cos(\Omega T_H/2)|1\rangle$.

From this state, the expectation value of $J_z^S = \sigma_z/2$ is $\langle J_z^S\rangle = (|\sin(\Omega T_H/2)|^2 - |\cos(\Omega T_H/2)|^2)/2 = (\sin^2(\Omega T_H/2) - \cos^2(\Omega T_H/2))/2 = -\cos(\Omega T_H)/2$. Since $(J_z^S)^2 = \mathbb{1}_2/4$ for a spin-$1/2$, the variance is ${\rm Var}(J_z^S) = \langle (J_z^S)^2\rangle - \langle J_z^S\rangle^2 = 1/4 - \cos^2(\Omega T_H)/4 = \sin^2(\Omega T_H)/4$, and its square root is $\sqrt{{\rm Var}(J_z^S)} = |\sin(\Omega T_H)|/2$. The derivative with respect to $\omega$ (with $\partial\Omega/\partial\omega = 1$) is $\partial\langle J_z^S\rangle/\partial\omega = (T_H/2)\sin(\Omega T_H)$. Substituting into the error-propagation formula $\Delta\omega = \sqrt{{\rm Var}(J_z^S)}/|\partial\langle J_z^S\rangle/\partial\omega|$ gives $\Delta\omega = (|\sin(\Omega T_H)|/2) / ((T_H/2)|\sin(\Omega T_H)|) = 1/T_H$. The factor $\sin(\Omega T_H)$ cancels identically wherever it is non-zero; at fringe extrema where $\sin(\Omega T_H) = 0$, the derivative vanishes and the sensitivity diverges — these isolated singular points (201 out of 40 401 grid points per $\omega$ in the numerical data) are measurement blind spots, not genuine deviations from SQL.

The argument fails for non-commuting drives such as $a_x J_x^A$ or $a_y J_y^A$ because $[J_z^A, a_x J_x^A] = i a_x J_y^A \neq 0$, so $[J_z^A, H] \neq 0$. The ancilla is no longer confined to a single $J_z^A$ eigenspace, $J_z^A(t)$ acquires nontrivial time dependence, the interaction term $a_{zz} J_z^S \otimes J_z^A$ cannot be replaced by a constant, and the system and ancilla become entangled during the hold, modifying the Ramsey-fringe visibility and destroying the exact cancellation that produces $\Delta\omega = 1/T_H$.
