...
Usage:
    uv run python reports/20260619/local.py --force
"""
...
def run_single_oat_optimisation(
    ...
    improvement = (
        delta_no_oat / best_nm.delta_omega_opt
        if np.isfinite(best_nm.delta_omega_opt)
        and best_nm.delta_omega_opt > 0
...
def _load_oat_checkpoints(
    checkpoint_dir: Path,
) -> tuple[set[tuple[int, int, float]], list[OATNScalingResult]]:
    ...
    for ckpt_file in sorted(checkpoint_dir.glob("N_*.parquet")):
        ...
        except (pd.errors.EmptyDataError, FileNotFoundError, KeyError) as exc:
            print(f"  [warn] Could not load checkpoint {ckpt_file}: {exc}")
    return completed, checkpoint_results


def _run_pending_oat_groups(
    items_to_run: list[tuple[int, int, float]],
    checkpoint_dir: Path,
) -> list[OATNScalingResult]:
    """Run pending (N_S, N_A, ω) groups in parallel, saving checkpoints.

    Args:
        items_to_run: List of (N_S, N_A, omega) triples to process.
        checkpoint_dir: Directory for saving checkpoint Parquet files.

    Returns:
        List of OATNScalingResult from newly completed runs.
    """
    # Group by (N_S, N_A) and process each group in parallel
    by_key: dict[tuple[int, int], list[tuple[int, int, float]]] = {}
    for N_S, N_A, omega in items_to_run:
        by_key.setdefault((N_S, N_A), []).append((N_S, N_A, omega))

    checkpoint_results: list[OATNScalingResult] = []
    for N_S, N_A in sorted(by_key):
        omega_items = by_key[(N_S, N_A)]
        n_ckpt = checkpoint_dir / f"N_{N_S:03d}_NA_{N_A:03d}.parquet"
        if n_ckpt.exists():
            print(f"  [ckpt] N_S={N_S}, N_A={N_A} already done, skipping")
            continue
        print(
            f"  [batch] N_S={N_S}, N_A={N_A}: {len(omega_items)} ω values (parallel)",
        )
        batch_results = parallel_map(
            _run_single_oat_for_parallel,
            omega_items,
            desc=f"N_S={N_S}, N_A={N_A} scan",
        )
        ckpt_list: list[OATNScalingResult] = []
        for rdict in batch_results:
            delta = rdict["delta_omega_opt"]
            if not np.isfinite(delta):
                print(
                    f"    [skip] N_S={rdict['N']}, N_A={rdict['n_ancilla']}, "
            ...
            ckpt_list.append(
                OATNScalingResult(
                    ...
                    q_opt=rdict["q_opt"],
                    delta_omega_no_oat=rdict["delta_omega_no_oat"],
                    improvement_factor=rdict["improvement_factor"],
        ...
        if ckpt_list:
            ckpt_list.sort(key=lambda r: r.omega)
            ckpt_scan = OATNScalingScanResult(results=ckpt_list)
...
def generate_oat_n_scaling_scan(
    ...
    if parquet_p.exists() and not force:
        ...
        completed, checkpoint_results = _load_oat_checkpoints(checkpoint_dir)
        ...
        if items_to_run:
            ...
            new_results = _run_pending_oat_groups(items_to_run, checkpoint_dir)
