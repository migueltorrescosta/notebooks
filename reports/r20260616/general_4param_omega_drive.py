...
Usage:
    uv run python reports/r20260616/general_4param_omega_drive.py --force
"""
...
def _combined_parallel_worker(args: tuple) -> tuple[int, np.ndarray]:
    """Worker for parallel 2D slice (parallel_grid_scan protocol).

    Args:
        args: Tuple ``(alpha_xx_chunk, alpha_zz_vals, start_idx, fixed_kwargs_dict)``.

    Returns:
        Tuple ``(start_idx, chunk_grid)``.
    """
    alpha_xx_chunk, alpha_zz_vals, start_idx, kw = args
    omega = kw["omega"]
    a_x = kw["a_x"]
    a_y = kw["a_y"]
    a_z = kw["a_z"]
    ancilla_dim = kw["ancilla_dim"]
...
def combined_2d_slice(
    ...
    if n_jobs is None or n_jobs == 1:
        ...
        def _sensitivity(a_xx: float, a_zz: float) -> float:
            return compute_combined_sensitivity(
                1,
                combined_initial_state(2 * ancilla_dim),
                T_BS,
                T_HOLD,
                omega,
                a_x,
                ...
                0.0,
                a_zz,
                build_full_ancilla_combined_operators(1),
                ancilla_dim,
            )

        grid = sequential_grid_scan(alpha_xx_vals, alpha_zz_vals, _sensitivity)
    else:
        grid = parallel_grid_scan(
            alpha_xx_vals,
            alpha_zz_vals,
            _combined_parallel_worker,
            n_jobs=n_jobs,
            omega=omega,
            a_x=a_x,
            a_y=a_y,
...
def _build_combined_result_from_row(row_dict: dict) -> CombinedOptimizationResult:
    return CombinedOptimizationResult(
        N=int(row_dict["N"]),
        ...
        alpha_xx_opt=float(row_dict["alpha_xx_opt"]),
        alpha_xz_opt=float(row_dict["alpha_xz_opt"]),
        alpha_zx_opt=float(row_dict["alpha_zx_opt"]),
        alpha_zz_opt=float(row_dict["alpha_zz_opt"]),
        expectation_Jz=float(row_dict["expectation_Jz"]),
        variance_Jz=float(row_dict["variance_Jz"]),
        t_hold=float(row_dict.get("t_hold", 10.0)),
        fd_step=float(row_dict.get("fd_step", 1e-6)),
        success=bool(int(row_dict.get("success", 0))),
        nfev=int(row_dict.get("nfev", 0)),
        n_starts=int(row_dict.get("n_starts", 0)),
        n_converged=int(row_dict.get("n_converged", 0)),
    )


def _combined_group_key(item: tuple[int, float, int]) -> int:
    return item[0]


def _make_combined_checkpoint_name(checkpoint_dir: Path) -> Callable:
    def _name_fn(key: int) -> Path:
        return checkpoint_dir / f"N_{int(key):03d}.parquet"
...
def generate_step2_n_scaling(force: bool = False) -> None:
    ...
    checkpoint_dir = (
        _REPORTS_DIR / f"r{_REPORT_DATE}" / "raw_data" / "checkpoints_step2"
    )
    ...
    completed, checkpoint_results = load_checkpoints(
        checkpoint_dir,
        _build_combined_result_from_row,
        ["N", "omega"],
    )
    ...
    if items_to_run:
        ...
        new_results = run_pending_groups(
            ...
            _make_result_from_dict,
            _combined_group_key,
            _make_combined_checkpoint_name(checkpoint_dir),
...
def generate_step3_n_scaling(force: bool = False) -> None:
    ...
    checkpoint_dir = (
        _REPORTS_DIR / f"r{_REPORT_DATE}" / "raw_data" / "checkpoints_step3"
    )
...
def main() -> None:
    ...
    for d in ["raw_data", "figures"]:
        (_REPORTS_DIR / f"r{_REPORT_DATE}" / d).mkdir(parents=True, exist_ok=True)
