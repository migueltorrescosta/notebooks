...
Usage:
    uv run python reports/20260620/local.py
    uv run python reports/20260620/local.py --force
"""
...
def compute_decoupled_baseline(
    ...
    """Compute the decoupled baseline sensitivity for :math:`N` system particles.

    At :math:`a_x = a_y = a_z = a_{zz} = 0`, the circuit reduces to a standard
    :math:`N`-particle MZI giving :math:`\\Delta\\omega = 1/(\\sqrt{N} T_{\\text{HOLD}})`.

    Args:
        N: Number of system particles.
        M: Number of ancilla particles (any M should produce the same SQL).
        omega_true: Phase rate value.

    Returns:
        :math:`\\Delta\\omega` at the decoupled configuration.
    """
    ops = build_operators(N, M)
    psi0 = free_initial_state(N, M, 0.0, 0.0)
    ...
    for N in N_values:
        ...
        for M in M_values:
            for omega in omega_values:
                delta = compute_decoupled_baseline(N, M, omega)
...
def _load_checkpoints(
    checkpoint_dir: Path,
) -> tuple[set[tuple[int, int, float]], list[FreeAncillaNScalingResult]]:
    ...
    checkpoint_results: list[FreeAncillaNScalingResult] = []
    expected_columns = FreeAncillaNScalingResult._PARQUET_COLUMNS
    for ckpt_file in sorted(checkpoint_dir.glob("N_*.parquet")):
        try:
            df_ckpt = pd.read_parquet(ckpt_file)
            # Fail fast if checkpoint is missing required columns
            missing = [c for c in expected_columns if c not in df_ckpt.columns]
            if missing:
                raise ValueError(
            ...
            for _, row in df_ckpt.iterrows():
                ...
                if np.isfinite(delta):
                    completed.add((n_val, m_val, w_val))
        ...
        except (pd.errors.EmptyDataError, FileNotFoundError, ValueError) as exc:
            print(f"  [warn] Could not load checkpoint {ckpt_file}: {exc}")
    return completed, checkpoint_results


def _run_pending_groups(
    items_to_run: list[tuple[int, int, float]],
    checkpoint_dir: Path,
) -> list[FreeAncillaNScalingResult]:
    """Run pending :math:`(N, M, \\omega)` groups in parallel, saving checkpoints per N.

    Args:
        items_to_run: List of ``(N, M, omega)`` triples to process.
        checkpoint_dir: Directory for saving ``N_*.parquet`` checkpoints.

    Returns:
        List of FreeAncillaNScalingResult from newly completed runs.
    """
    # Group by N and process each group in parallel
    by_N: dict[int, list[tuple[int, int, float]]] = {}
    for N, M, omega in items_to_run:
        by_N.setdefault(N, []).append((N, M, omega))

    checkpoint_results: list[FreeAncillaNScalingResult] = []
    for N in sorted(by_N):
        n_m_omega_items = by_N[N]
        n_ckpt = checkpoint_dir / f"N_{N:03d}.parquet"
        if n_ckpt.exists():
            print(f"  [ckpt] N={N} already done, skipping")
            continue
        print(f"  [batch] N={N}: {len(n_m_omega_items)} (M, ω) pairs (parallel)")
        batch_results = parallel_map(
            _run_single_n_m_omega_worker,
            n_m_omega_items,
            desc=f"N={N} scan",
        )
        ckpt_list: list[FreeAncillaNScalingResult] = []
        for rdict in batch_results:
            delta = rdict["delta_omega_opt"]
            if not np.isfinite(delta):
                print(
                    f"    [skip] N={rdict['N']}, M={rdict['M']}, "
                    f"ω={rdict['omega']}: Δω={delta}",
                )
                continue
            ckpt_list.append(
                FreeAncillaNScalingResult(
                    ...
                    sql=rdict["sql"],
                    ratio=rdict["ratio"],
                    theta_A_opt=rdict["theta_A_opt"],
                    phi_A_opt=rdict["phi_A_opt"],
                    ...
                    nfev=rdict["nfev"],
                    J_A=rdict["J_A"],
                    drive_norm=rdict["drive_norm"],
                    d_exp=rdict["d_exp"],
                    is_fringe=bool(rdict["is_fringe"]),
                ),
            )
        if ckpt_list:
            ckpt_scan = FreeAncillaNScalingScanResult(results=ckpt_list)
...
def generate_n_scaling_scan(force: bool = False) -> None:
    ...
    if parquet_p.exists() and not force:
        ...
        if items_to_run:
            ...
            new_results = _run_pending_groups(items_to_run, checkpoint_dir)
