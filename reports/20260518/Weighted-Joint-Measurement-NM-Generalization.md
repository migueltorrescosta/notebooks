...
- **N-scaling fitter** — Weighted log-log regression with bootstrap CI (10$^4$ resamples), curvature diagnostic $\beta$ with bootstrap CI, per-seed statistics in `NScalingResult.delta_omega_seeds`. Tests: 10 (dataclass, bootstrap CI, `to_dataframe()`, weighted linear/quadratic fit, CSV roundtrip, etc.). — PASS
