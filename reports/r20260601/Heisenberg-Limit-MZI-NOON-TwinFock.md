...
For a standard Mach-Zehnder interferometer (MZI; BS1, phase shift, BS2) where the phase is generated by the angular momentum operator $J_z = (n_1 - n_2)/2$ with holding time $H_t = 10$:
...
*Interference pattern: $\langle J_z\rangle$ (solid line) and $\pm\sigma$ band (shaded) versus $\omega$ for NOON. The N=1 state (blue) shows the familiar single-photon MZI fringe, while all N>1 states (orange, N=40 shown) collapse to $\langle J_z\rangle = 0$ identically — the degenerate first moment that motivates the CFI approach. The shaded ±σ band for N=40 is negligible at this scale (Var($J_z$) = N/4).*
...
The **measurement** is a number-difference detection in the Fock basis at the output. We expand the output state in the two-mode Fock basis and compute the probability of each number-difference $m = n_1 - n_2$ with:

$P(m\vert\omega) = \sum_{n_1, n_2: n_1-n_2=m} \vert\langle n_1, n_2 \vert \psi_{\text{out}}\rangle\vert^2.$

The sensitivity is computed from the **classical Fisher information** of the full distribution:
...
The **quantum Fisher information** provides the ultimate lower bound from the quantum Cramér-Rao inequality $\Delta\omega_C \ge \Delta\omega_Q = 1/\sqrt{F_Q}$. For pure states under a fixed generator $G = H_t \cdot J_z$, the QFI is $F_Q = 4 \cdot \text{Var}(G)_{\text{probe}} = 4 H_t^2 \cdot \text{Var}(J_z)_{\text{probe}}$, which is $\omega$-independent and depends only on the probe state.
...
3. **Sweep structure** — Nested loops over $(N, \omega)$ produce a 2D grid of sensitivity values. For each $(N, \omega)$:
   ...
   - Evaluate $F_C(\omega) = \sum_m (\partial P/\partial\omega)^2 / P$ by central finite differences with $\varepsilon = 10^{-6}$
...
All data generated with $H_t = 10$, $\omega \in [0.1, 5.0]$ (step 0.1), and $N$ up to 40. The Classical Fisher Information is computed from $P(m\vert\omega)$ by central finite differences with $\varepsilon = 10^{-6}$.
...
- This matches the theoretical QFI exponent for this finite $N$ range (which is $\alpha_Q = -0.932$ from the $(N+2)$ correction)
...
*Phase sensitivity scaling: $\Delta\omega$ vs $N$ for NOON (circles) and Twin-Fock (squares). Both states lie well below the standard quantum limit (SQL) (dotted line). NOON follows the Heisenberg limit $1/(H_t N)$ (dashed line). Twin-Fock shows a slightly shallower slope ($\alpha=-0.932$) because of the $N(N+2)$ finite-size correction, asymptotically approaching $\alpha\to-1.0$.*
