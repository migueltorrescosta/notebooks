...
Usage:
    uv run python reports/20260612/local.py --force
"""
...
from src.analysis.n_scaling_result import (
    NScalingResult as MultiNScalingResult,
)
from src.analysis.n_scaling_result import (
    NScalingScanResult as MultiNScalingScanResult,
)
...
def multi_particle_random_search(
    N: int,
    omega: float,
    ...
    for i in range(n_samples):
        ...
        domega = compute_multi_particle_sensitivity(
            N,
            psi0,
            T_BS,
            T_HOLD,
...
def multi_particle_sensitivity_objective(
    params: np.ndarray,
    N: int,
    omega_true: float,
    ops: dict[str, np.ndarray],
    ...
    """Objective function for minimising Delta_omega in the multi-particle protocol.
    ...
    Returns:
        Delta_omega (plus infinite penalty if bounds violated).
    """
    ax = float(params[0])
    ay = float(params[1])
    az = float(params[2])
...
def run_multi_particle_nelder_mead(
    N: int,
    omega_true: float,
    ...
    """Run Nelder-Mead optimisation for the multi-particle omega-modulated protocol.

    Args:
        N: Number of particles per subsystem.
        omega_true: True phase rate parameter.
        ops: Multi-particle operators (built if None).
    ...
    def objective(p: np.ndarray) -> float:
        return multi_particle_sensitivity_objective(
            p,
            N,
            omega_true,
            ops,
    ...
    psi_final = evolve_multi_particle_circuit(
        N,
        psi0,
        T_BS,
        T_HOLD,
...
def run_single_n_omega(
    ...
    Returns:
        MultiNScalingResult with the optimal parameters and sensitivity.
    """
    ...
    rs_result = multi_particle_random_search(
        N,
        omega,
        n_samples=N_RANDOM,
        seed=base_seed,
    ...
    for rank, idx in enumerate(top_indices):
        ...
        nm = run_multi_particle_nelder_mead(
            N=N,
            omega_true=omega,
            ops=ops,
            psi0=psi0,
...
def generate_n_scaling_scan(force: bool = False) -> None:
    ...
    """
    parquet_p = _parquet_path("n-scaling-scan")
    ...
    if parquet_p.exists() and not force:
        ...
        summary = MultiNScalingScanResult.from_parquet(parquet_p)
    else:
        if force:
        ...
        checkpoint_results: list[MultiNScalingResult] = []
        ...
        # Process N values sequentially (with omega parallelism within each N)
        items_to_run = [
            (N, omega)
            for N in N_VALS
        ...
        if items_to_run:
            print(
                f"[run] N-scaling scan: {len(items_to_run)} remaining (N, omega) pairs"
            )
            ...
            for N in sorted(by_N):
                ...
                if n_ckpt.exists():
                    ...
                    continue
                print(f"  [batch] N={N}: {len(omega_items)} omega values (parallel)")
                ...
                # Save checkpoint
                ckpt_list: list[MultiNScalingResult] = []
                for rdict in batch_results:
                    delta = rdict["delta_omega_opt"]
                    if not np.isfinite(delta):
                        print(
                            f"    [skip] N={rdict['N']}, "
                            f"omega={rdict['omega']}: Delta_omega={delta}"
                        )
                        continue
                    ckpt_list.append(
                        MultiNScalingResult(
                ...
                if ckpt_list:
                    ckpt_scan = MultiNScalingScanResult(results=ckpt_list)
        ...
        summary = MultiNScalingScanResult(results=checkpoint_results)
...
def generate_decoupled_baseline(force: bool = False) -> None:
    """Verify the decoupled baseline for all N and omega values."""
    ...
    print("[run] Computing decoupled baseline for all (N, omega)...")
    verifications = verify_multi_particle_decoupled_baseline()
