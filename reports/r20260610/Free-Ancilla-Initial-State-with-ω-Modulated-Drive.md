...
- **Free-ancilla $\omega$-modulated objective** — A 6D objective function wrapper that takes $(\theta_A, \phi_A, a_x, a_y, a_z, a_{zz})$ and computes $\Delta\omega$ using the $\omega$-modulated Hamiltonian combined with the free-ancilla initial state. Core computations already exist in both `reports/r20260519/phase_modulated_drive.py` and `reports/r20260528/free_ancilla_initial_state.py`; this is a composition step.
