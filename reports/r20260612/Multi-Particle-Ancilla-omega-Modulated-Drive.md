...
beats the $N$-particle standard quantum limit (SQL) at all $N\in[1,20]$, but the SQL-violation ratio decays as
...
from `src.physics.dicke_basis`. These are embedded into the full $(N+1)^2$ space by
Kronecker products:
...
1. **Beam splitter on system only**: A 50/50 symmetric beam splitter acts on the
   system through $U_{\text{BS}}^{(S)} = \exp(-i(\pi/2) J_x^S) =
...
2. **Holding period**: The full state evolves under
   ...
   The hold unitary is $U_{\text{hold}}(T_H) = \exp(-i T_H H)$ computed by
...
The derivative is computed by central finite differences with step $\delta=10^{-6}$,
...
The quantum Fisher information (QFI) for a pure state $|\psi(\omega)\rangle$ generated by Hamiltonian $H$ is
...
$H_{\text{int}}$. The Baker-Campbell-Hausdorff (BCH) expansion of $U_{\text{hold}} = \exp(-i T_H H)$ generates
...
1. **Operator construction** — Build $J_z$, $J_x$, $J_y$ as $(N+1)\times(N+1)$
   ...
   $(N+1)^2\times(N+1)^2$ ancilla operators by
   $J_k^A = \mathbb{1}_{N+1} \otimes J_k(N)$ and system operators by
...
4. **Hold unitary** — Compute $U_{\text{hold}}(T_H) = \exp(-i T_H H)$ by
...
5. **Sensitivity computation** — $\Delta\omega = \sqrt{\text{Var}(J_z^S)}
   / |\partial\langle J_z^S\rangle/\partial\omega|$ by central finite
...
500 random evaluations + 50 Nelder--Mead refinements. The maximum matrix dimension $441\times441$
($N=20$) gives about 5 ms per matrix exponential, making the total runtime
comparable to #20260611 (about 1 to 2 hours with parallel dispatch).
...
- **SQL baseline recovery**: At $a_x = a_y = a_z = a_{zz} = 0$, the circuit reduces to a standard $N$-particle Mach-Zehnder interferometer (MZI) with $\Delta\omega = 1/(\sqrt{N} T_H)$. Verified for all $N$ and $\omega$.
...
The implementation is a direct modification of `reports/r20260611/n_scaling_phase_modulated.py`. The changes are confined to operator construction and dimension management:
...
| **Computational cost at $N=20$** — The $441\times441$ matrix exponential is about 600 times larger than the $4\times4$ case, each evaluation taking about 5 ms. With 3 evaluations per objective (central diff) and $500 + 50\cdot N_{\text{iter}}$ evaluations per $(N,\omega)$ pair, the runtime may be about 5 to 10 minutes per pair. | Use parallel dispatch across $(N,\omega)$ pairs. The $N=20$ case dominates runtime; consider limiting the full scan to $N\le 15$ and running $N=20$ separately. |
...
**Parameter saturation**: A minor fraction of parameters saturate at the $[-5, 5]$ bounds — 1/40 for $a_x$, 3/40 for $a_z$, 3/40 for $a_{zz}$, and 0/40 for $a_y$. The saturation is concentrated at $N=1$ (four of seven saturated instances), suggesting larger bounds may not unlock significant improvement at N>1.
...
**Summary**: six criteria PASS, one FAIL (scaling exponent), one PARTIAL (arrested ratio decay — confirmed for N ≤ 8 but N=9–20 pending), one PENDING (definitively verifying $R(20)\geq2$ requires completing the N=9–20 scan). The primary hypothesis (the multi-particle ancilla arrests the ratio decay and beats SQL at all N) is supported in the available N range. The secondary hypothesis (Heisenberg-limit scaling) is rejected. The next logical step is to complete the N=9–20 scan to confirm the arrested-ratio-decay finding across the full range, and then extend to a joint measurement protocol that may unlock the Heisenberg scaling.
