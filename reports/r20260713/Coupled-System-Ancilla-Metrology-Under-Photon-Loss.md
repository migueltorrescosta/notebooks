...
1. **Beam splitter on system only**: A 50/50 symmetric beam splitter $U_{\text{BS}} = \exp(-i (\pi/2) J_x)$ acts on the S subsystem. The full-space operator is $U_{\text{BS}}^{(S)} = U_{\text{BS}} \otimes \mathbb{1}_{(N+1)^2}$. The multi-particle BS matrix elements are computed by block-diagonal matrix exponentiation over constant-total-photon subspaces.
...
- **Coupling QFI ratio**: $\mathcal{R}_{QFI}(\gamma, N) = F_Q^{(C)} / (F_Q^{(S)} + F_Q^{(A)}) = F_Q^{(C)} / (2 F_Q^{(A)})$ — measures whether the four-parameter coupling creates information beyond two independent resources. $\mathcal{R}_{QFI} > 1$ means the coupling increases the total information; $\mathcal{R}_{QFI} = 1$ means the coupling leaves the total information unchanged; $\mathcal{R}_{QFI} < 1$ means the coupling reduces the total information.
- **Practical EP ratio**: $\mathcal{R}_{EP}(\gamma, N, \omega) = \Delta\omega_{EP}^{(C)} / \Delta\omega_{EP}^{(A)}$ — measures the practical S-only sensitivity improvement from adding a coupled ancilla. $\mathcal{R}_{EP} < 1$ means the coupling and ancilla together improve the sensitivity.
...
- $\gamma$-sweep: $8 \times 61 = 488$ points, each requiring three Config A and three Config C Lindblad evaluations = $2{,}928$ Lindblad solves of dimension $(N+1)^4$ — bottleneck at $N=6$-$8$
...
**Estimated wall time**: With `joblib.Parallel(n_jobs=-1)`, the $\gamma$-sweep for $N = 1$--$3$ runs in less than 10 minutes each; $N = 4$ about 30 minutes; $N = 5$ about 3 hours; $N = 6$ about 17 hours; $N = 7$ about 70 hours; $N = 8$ about 250 hours (single-threaded estimates). With full parallelisation across all $\gamma$ values, the total wall time is about 24 hours for the complete sweep.
...
**Key Finding**: Config A recovers the SQL $\Delta\omega = 1/(\sqrt{N} T_H)$ exactly at $\gamma = 0$ for all $N$, confirming the noiseless MZI baseline. The $1/\sqrt{N}$ scaling holds across the full $N = 1$–$8$ range.
...
The v2 simulation run is pending. The v1 run (seven $\gamma$ values, bounds $[-5, 5]^4$) found 6/7 criteria PASS and one PARTIAL (monotonic degradation). The v2 run extends Config C to $N = 1$–$8$, uses 61 $\gamma$ values, and widens bounds to $[-10, 10]^4$ to resolve the boundary-saturation issue.
...
**v2 changes** (61 $\gamma$ values, bounds $[-10, 10]^4$, Config C $N = 1$–$8$): The v2 run addresses three limitations of v1: (a) it extends Config C from $N = 1$–$3$ to the full $N = 1$–$8$ range, enabling direct comparison of the coupling advantage across all accessible particle numbers; (b) it refines the $\gamma$ grid from seven to 61 values (log-spaced in $[10^{-6}, 10^6]$), providing finer resolution of the noise-dependent coupling advantage; (c) it widens the coupling bounds from $[-5, 5]^4$ to $[-10, 10]^4$, resolving the boundary-saturation issue at $N = 2$.
