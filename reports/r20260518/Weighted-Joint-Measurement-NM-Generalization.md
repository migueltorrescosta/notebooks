...
For a system S of N particles and an ancilla A of M particles (each in two-mode bosonic Fock spaces), where only S couples to the unknown phase rate $\omega$ through $H_{\text{enc}} = \omega J_z^S$, and S and A interact through $H_{\text{int}} = \alpha_{xx} J_x^S \otimes J_x^A + \alpha_{xz} J_x^S \otimes J_z^A + \alpha_{zx} J_z^S \otimes J_x^A + \alpha_{zz} J_z^S \otimes J_z^A$ during a holding time $t_hold$, the **weighted joint measurement** $M(a,b) = a\,J_z^S + b\,J_z^A$ with $a^2 + b^2 = 1$ (L$_2$-normalised) can exploit S-A correlations to improve sensitivity beyond the S-only measurement.

The central hypotheses are:

1. **Optimal weights unlock enhanced sensitivity.** For any $N \ge 1$, $M \ge 1$, and non-zero interaction, there exists an optimal pair $(a^*, b^*)$ with $a^{*2} + b^{*2} = 1$ that minimises $\Delta\omega$. The equal-weight case $(a,b) = (1/\sqrt{2}, 1/\sqrt{2})$ from previous work is generally suboptimal. The optimal weight ratio $a^*/b^*$ is determined by the relative magnitudes of $\text{Var}(J_z^S)$, $\text{Var}(J_z^A)$, and $\text{Cov}(J_z^S, J_z^A)$, and varies with $\alpha_{ij}$, $N$, $M$, and $t_hold$.

2. **N-scaling beyond the standard quantum limit (SQL) is accessible through the joint measurement.** With optimised weights and appropriate interaction, the sensitivity scales as $\Delta\omega \propto 1/(N^\nu t_hold)$ with $\nu > 1/2$ (beating the SQL scaling $\nu = 1/2$) and approaching $\nu \to 1$ (Heisenberg scaling) in the limit of large $N$ with ideal interaction. This is possible because the weighted joint measurement can extract variance from both S and A while the signal derivative $\partial\langle M\rangle/\partial\omega$ scales with $N$ through $J_z^S$.

3. **Ancilla size M contributes diminishing returns.** For fixed $N$, increasing $M$ beyond $M \sim N$ provides negligible improvement, because $\omega$ couples only to $J_z^S$ (operator norm $N/2$) and the ancilla serves only as a readout-enhancement resource. The interaction strength $\alpha_{ij}$ must be large enough to distribute $\omega$-dependent phase information from S to A; a larger $M$ helps only if the interaction effectively couples S to all $M$ ancilla particles.

4. **The quantum Fisher information (QFI) bound remains $\Delta\omega \ge 1/(N t_hold)$**, set by the maximal eigenvalue $J_S = N/2$ of the generator $J_z^S$. The ancilla and the weighted measurement cannot surpass this Heisenberg limit, but can saturate it in the ideal interaction regime.
...
The **collective angular momentum operators** for each subsystem are obtained from standard SU(2) algebra. For a spin-$J$ system, $J_z$ is diagonal with eigenvalues $m \in \{-J, -J+1, \dots, J\}$, $J_x$ has matrix elements $\langle J, m' \vert J_x \vert J, m \rangle = \tfrac12\sqrt{J(J+1) - m(m\pm 1)}\,\delta_{m', m\pm 1}$, and $J_y$ is defined analogously. These operators satisfy $[J_x, J_z] = i J_y$ and are efficiently constructed with `dicke_basis.jz_operator(N)` and `dicke_basis.jx_operator(N)` from the existing codebase. They are embedded into the full space by Kronecker products: $J_z^S = J_z(N) \otimes \mathbb{1}_{M+1}$, $J_x^S = J_x(N) \otimes \mathbb{1}_{M+1}$, $J_z^A = \mathbb{1}_{N+1} \otimes J_z(M)$, $J_x^A = \mathbb{1}_{N+1} \otimes J_x(M)$.
...
where $\Theta \in [0, \pi]$ and $\Phi \in [0, 2\pi)$. The CSS family has the structure of a generalised Bloch sphere for spin $J$, and reduces to the standard single-qubit parameterisation for $J=1/2$ (that is, $N=1$). **A gauge freedom** — global phase invariance around $J_z$ before the first beam splitter, combined with the beam splitter's ability to generate any in-plane rotation — allows us to fix $\Phi_S = 0$ and $\Phi_A = 0$ without loss of generality. This reduction from 4 to 2 initial-state degrees of freedom is conservative: it assumes the first beam-splitter rotation $e^{-i T_{\text{BS}1} J_x}$ explores the same reachable set as the full $(\Theta, \Phi)$ parameterisation with a second $J_x$-rotation. If this symmetry turns out to be only approximate (for example, for Hamiltonians that break the $J_z$-rotation symmetry of the initial state), the full $(\Theta, \Phi)$ parameterisation can be restored.

The **circuit** proceeds in three steps. First, a **beam-splitter unitary** generated by $J_x$ acts independently on S and A: $U_{\text{BS}}(T) = \exp(-i T J_x)$, applied as $U_{\text{BS}}^{(2)}(T) = U_{\text{BS}}(T; N) \otimes U_{\text{BS}}(T; M)$ with independent times $T_{\text{BS}1}$ (before the hold) and $T_{\text{BS}2}$ (after the hold). The $U_{\text{BS}}(T)$ for each subsystem is computed by `scipy.linalg.expm(-1j * T * J_x)`.

Second, during the **holding time** $t_hold$, the system evolves under
$H_{\text{hold}} = \omega\,(J_z^S) + H_{\text{int}},$
where $H_{\text{int}} = \alpha_{xx} J_x^S \otimes J_x^A + \alpha_{xz} J_x^S \otimes J_z^A + \alpha_{zx} J_z^S \otimes J_x^A + \alpha_{zz} J_z^S \otimes J_z^A.$ Crucially, **only the system S accumulates phase from $\omega$**; the ancilla experiences no $\omega$-dependent evolution. The hold unitary is $U_{\text{hold}}(t_hold) = \exp(-i t_hold H_{\text{hold}})$, computed by `torch.linalg.matrix_exp` acting on the $d$-dimensional space (the PyTorch implementation enables automatic differentiation (AD) through this operation). Third, the second beam splitter is applied. The complete evolution is
$\vert\Psi_{\text{final}}\rangle = U_{\text{BS}}^{(2)}(T_{\text{BS}2})\, U_{\text{hold}}(t_hold)\, U_{\text{BS}}^{(2)}(T_{\text{BS}1})\, (\vert\psi_S\rangle \otimes \vert\psi_A\rangle).$
...
where $\text{Cov}(J_z^S, J_z^A) = \langle J_z^S J_z^A\rangle - \langle J_z^S\rangle\langle J_z^A\rangle$. The **sensitivity** by error propagation is $\Delta\omega = \sqrt{\text{Var}(M)} / \vert\partial\langle M\rangle/\partial\omega\vert$. The weight angle $\psi$ is **not optimised by the outer gradient-based solver**; instead, at each circuit-parameter evaluation, $\psi^*$ is found by a 1D golden-section search over $\psi \in [0, 2\pi)$ that minimises $\Delta\omega(\psi)$ using the six pre-computed moments. This sub-optimisation is exact and fast (no matrix exponentials needed per $\psi$-evaluation). The outer optimiser sees the objective $f(\text{params}) = \min_\psi \Delta\omega(\psi, \text{params})$, and the gradient $\nabla f$ is obtained by automatic differentiation through the circuit at the optimal $\psi^*$, applying the envelope theorem: $\partial f / \partial p_i = \partial \Delta\omega / \partial p_i \vert_{\psi=\psi^*}$ because $\partial \Delta\omega / \partial \psi \vert_{\psi=\psi^*} = 0$.

The **QFI bound** follows from the same argument as the $N=M=1$ case. The generator of $\omega$ is $J_z^S$, with operator norm $\|J_z^S\| = J_S = N/2$. For a pure initial state $\vert\Psi_0\rangle$, the symmetric logarithmic derivative at $\omega=0$ is $h_0 = \int_0^{t_hold} e^{i t H_{\text{int}}} J_z^S e^{-i t H_{\text{int}}} \, dt$. Since $\|e^{i t H_{\text{int}}} J_z^S e^{-i t H_{\text{int}}}\| = \|J_z^S\| = N/2$ for all $t$, the operator norm of $h_0$ is at most $t_hold N/2$. Consequently $\text{Var}(h_0) \leq (t_hold N/2)^2$, giving $F_Q \leq t_hold^2 N^2$, and the Cramér–Rao bound implies
$\Delta\omega \geq \frac{1}{\sqrt{F_Q}} \geq \frac{1}{N t_hold}.$
This is the **Heisenberg limit** (HL) for $N$ particles. The joint measurement with optimised weights respects this bound. The SQL ratio $r_{\text{SQL}} = \Delta\omega \sqrt{N} t_hold$ is tracked as a diagnostic benchmark: $r_{\text{SQL}} < 1$ indicates beating the SQL. It is never asserted.
...
| $t_hold$ | Holding time | $[0.1, 20]$ |
| $\alpha_{ij}$ | Interaction coefficients | $[-2, 2]$ |
| $\psi$ | Measurement weight angle (sub-optimised, not in outer loop) | $[0, 2\pi)$ |
| $\omega$ (true) | Phase rate | 1.0 (default) |
...
1. **Zero interaction** ($\alpha_{ij} = 0$): The system and ancilla evolve independently. The weighted measurement sensitivity reduces to:
   $\Delta\omega = \frac{\sqrt{a^2\,\text{Var}(J_z^S) + b^2\,\text{Var}(J_z^A)}}{\vert a\vert\,\vert\partial\langle J_z^S\rangle/\partial\omega\vert},$
   where the ancilla contributes only noise (zero signal derivative because $\partial\langle J_z^A\rangle/\partial\omega = 0$). The optimal weight is $a^* = 1$, $b^* = 0$ (S-only measurement), and the sensitivity cannot beat SQL. The numerical pipeline must reproduce this exactly (relative error $< 10^{-10}$).

2. **$\alpha_{zz}$-only interaction** ($\alpha_{xx} = \alpha_{xz} = \alpha_{zx} = 0$, $\alpha_{zz} \neq 0$): The hold Hamiltonian is $H_{\text{hold}} = \omega J_z^S + \alpha_{zz} J_z^S \otimes J_z^A$. Since $[J_z^S \otimes J_z^A, J_z^S] = 0$, the evolution is **diagonal in the product Dicke basis** $\vert m_S, m_A\rangle$. The phase $\omega$ is imprinted as $e^{-i(\omega + \alpha_{zz} m_A) t_hold m_S}$ — a simple phase per $m_S$ eigenvalue. All expectation values, variances, and covariances can be computed analytically as explicit functions of $N, M, \alpha_{zz}, t_hold$. The numerical pipeline must reproduce these exact expressions (relative error $< 10^{-10}$).
...
1. **Operator construction** — Build $J_z(N)$, $J_x(N)$, $J_y(N)$ for spin $J=N/2$ with `dicke_basis.jz_operator(N)` and `dicke_basis.jx_operator(N)` (which return $(N+1)\times(N+1)$ numpy arrays). Embed into the full $(N+1)(M+1)$-dimensional space by Kronecker products: $J_z^S = J_z(N) \otimes \mathbb{1}_{M+1}$, $J_x^S = J_x(N) \otimes \mathbb{1}_{M+1}$, $J_z^A = \mathbb{1}_{N+1} \otimes J_z(M)$, $J_x^A = \mathbb{1}_{N+1} \otimes J_x(M)$. Construct $H_{\text{int}}$ as a linear combination of the four tensor products, and the measurement operator $M(a,b) = a J_z^S + b J_z^A$ where $(a,b) = (\cos\psi, \sin\psi)$. Convert all operators to `torch.tensor(dtype=torch.complex128)` for the AD-enabled computation graph.

2. **State preparation** — Prepare each subsystem as a CSS with $\Phi_S = \Phi_A = 0$ (azimuth gauge fixing). Use the Dicke-basis rotation: $\vert\Theta, 0\rangle = \exp(-i\Theta J_y)\,\vert J, -J\rangle$. The ground state $\vert J, -J\rangle$ is the last basis vector in the Dicke ordering $m = J, J-1, \dots, -J$. The rotation exponential is computed by `torch.linalg.matrix_exp`. The full state is the Kronecker product $\vert\Psi_0\rangle = \vert\psi_S\rangle \otimes \vert\psi_A\rangle$.

3. **Beam-splitter unitaries** — Compute $U_{\text{BS}}(T; J) = \exp(-i T J_x)$ for each subsystem by `torch.linalg.matrix_exp`. The combined unitary is $U_{\text{BS}}^{(2)}(T) = U_{\text{BS}}(T; N) \otimes U_{\text{BS}}(T; M)$ for independent $T_{\text{BS}1}$ and $T_{\text{BS}2}$.

4. **Hold unitary** — Compute $U_{\text{hold}}(t_hold) = \exp(-i t_hold H_{\text{hold}})$ by `torch.linalg.matrix_exp` on the full $d \times d$ matrix. $H_{\text{hold}}$ includes the $\omega$-dependent term $\omega J_z^S$ and the interaction $H_{\text{int}}$.

5. **Forward simulation function** — Define a differentiable PyTorch function that takes the 9 circuit parameters ($\Theta_S, \Theta_A, T_{\text{BS}1}, T_{\text{BS}2}, t_hold, \alpha_{xx}, \alpha_{xz}, \alpha_{zx}, \alpha_{zz}$), computes $\vert\Psi_{\text{final}}\rangle$ at $\omega$ and $\vert\Psi_{\text{final}}^{\pm}\rangle$ at $\omega \pm \delta$ (for the derivative), and returns the six moments ($\langle J_z^S\rangle, \langle J_z^A\rangle, \text{Var}(J_z^S), \text{Var}(J_z^A), \text{Cov}(J_z^S, J_z^A)$) and their $\omega$-derivatives computed by central finite differences with step $\delta = 10^{-6}$.

6. **Weight sub-optimisation** — Given the six moments and their $\omega$-derivatives, compute $\Delta\omega(\psi)$ analytically for any $\psi$. Perform a golden-section search over $\psi \in [0, 2\pi)$ to find $\psi^*$ that minimises $\Delta\omega$. This is fast (no matrix exponentials, pure arithmetic). The optimal objective value is $f_{\text{params}} = \Delta\omega(\psi^*)$.

7. **Gradient computation** — Compute gradients of $f_{\text{params}}$ with respect to all 9 circuit parameters by `torch.autograd.grad`. The gradients are evaluated at $\psi^*$ (envelope theorem: $\partial\Delta\omega(\psi^*)/\partial p = \partial\Delta\omega/\partial p\vert_{\psi=\psi^*}$ since $\partial\Delta\omega/\partial\psi\vert_{\psi^*} = 0$). The finite-difference step for $\omega$ is applied as a non-differentiable preprocessing step (the $\pm\delta$ evaluations are detached from the gradient graph), while all matrix exponentials and state evolution remain in the graph.

8. **Gradient-based optimisation** — Use `scipy.optimize.minimize(method='L-BFGS-B')` with bounds on all parameters. At each iteration, scipy provides a candidate parameter vector; convert to torch tensors, run the forward+sub-optimisation, extract $f$ and $\nabla f$ by AD, and return both to scipy. L-BFGS-B respects bound constraints (for example, $\Theta_S \in [0, \pi]$, $t_hold \in [0.1, 20]$, $\alpha_{ij} \in [-2, 2]$). Use multiple random starting points (10+ seeds per $(N,M)$ configuration) to mitigate local-minima risk; the gradient-based search within each basin handles the high-dimensional landscape efficiently.

9. **Analytical benchmark validation** — Validated against the two analytical benchmarks (§ ⚛️, sub-section "Analytical Benchmarks"). The zero-interaction benchmark reproduces exact SO(3)-rotation formulas to within $10^{-10}$ relative error across $N \in \{1,2,4,8\}$ and $t_hold \in \{0.5, 1.0, 2.0\}$ (configurations parametrised in `TestAnalyticalBenchmarks`). The $\alpha_{zz}$-only benchmark reproduces the diagonal-form moment expressions to within $10^{-10}$ relative error across $N \in \{1,2,4,8\}$ and $\alpha_{zz} \in \{0.5, 1.0, 2.0\}$. Both validations pass as part of the 2064+ test regression suite.

10. **Statistical scaling extraction** — After optimisation converges per $(N,M)$, collect all $K \ge 20$ per-seed $\Delta\omega$ values (default `num_seeds=20` in `run_n_scaling`; raise further for production runs). Compute the per-$N$ mean $\overline{\Delta\omega}_N$ and standard deviation $\sigma_N$, then perform a **weighted log-log regression**: $\log\overline{\Delta\omega} = -\nu \log N + c$ with weights $w_N = 1/\sigma_N^2$. The weighted fit uses the design matrix $A = [-\log N, \mathbf{1}]$ and minimises $\sum w_N (\log\overline{\Delta\omega}_N + \nu\log N - c)^2$. A **bootstrap** procedure resamples the $(N, \overline{\Delta\omega}_N, \sigma_N)$ tuples with replacement (10$^4$ resamples), recomputing $\nu$ per resample using the same weighted regression. Report $\nu$ as the **median of the bootstrap distribution** with a **95% confidence interval** (2.5–97.5 percentiles). Also fit a weighted quadratic model $\log\Delta\omega = -\nu \log N + \beta(\log N)^2 + c$ and bootstrap its $\beta$ coefficient as a curvature diagnostic — a non-zero $\beta$ indicates finite-size corrections are significant.

### Parameter Sweep

| Sweep Type | Parameters Scanned | Fixed Parameters | Purpose |
|------------|-------------------|------------------|---------|
| Weight optimisation | $\psi$ (golden-section) | Any circuit params | Find $(a^*, b^*)$ at each circuit configuration |
...
| $t_hold$ bound investigation | $t_hold \in [0.1, 20]$ | $N$, $M$, fully optimised | Verify $\Delta\omega \propto 1/t_hold$ with weights |
...
- **Heisenberg bound**: $\Delta\omega \cdot N \cdot t_hold \ge 1$ (unconditional; tolerance $10^{-6}$).
- **CSS normalisation**: $\langle\Theta,0\vert\Theta,0\rangle = 1$ for all subsystem CSS states.
- **Analytical benchmark: zero interaction**: When $\alpha_{xx} = \alpha_{xz} = \alpha_{zx} = \alpha_{zz} = 0$, the optimal weight is $a^* = 1$, $b^* = 0$ (S-only measurement); the sensitivity obeys $\Delta\omega \ge 1/(\sqrt{N} t_hold)$ (cannot beat SQL without interaction); and the numerical result matches the closed-form expression to within $10^{-10}$ relative error.
...
- **$N=M=1$ regression**: At $a = b = 1/\sqrt{2}$, the sensitivity recovers $\Delta\omega = 1/t_hold$ (previous report's equal-weight result) to within $0.1\%$ relative error.
- **SQL ratio** (diagnostic, not an assertion): $r_{\text{SQL}} = \Delta\omega \sqrt{N} t_hold$ is tracked; $r_{\text{SQL}} < 1$ indicates SQL beating. Never asserted.

#### 🔧 Implementation Status

- **Collective operators** — $J_z(N)$, $J_x(N)$, $J_y(N)$ for arbitrary $N$ with `dicke_basis`. Tests: 10 unit (eigenvalues, commutation, embedding, torch conversion). — PASS
...
- **Weight sub-optimisation** — Golden-section search over $\psi$ at fixed circuit parameters. Tests: 3 integration (matches brute-force sweep, equal-weight symmetry, $\psi$ wrapping). — PASS
...
- **Hold unitary (torch)** — $\exp(-i t_hold [\omega J_z^S + H_{\text{int}}])$ by `torch.linalg.matrix_exp`. Tests: 4 integration (norm preservation, identity, scipy consistency). — PASS
...
- **Sensitivity** — $\Delta\omega = \sqrt{\text{Var}(M)} / \vert\partial\langle M\rangle/\partial\omega\vert$ with covariance. Tests: 6 integration (moments, covariance, decoupled, fringe extremum, weighted beats S-only). — PASS
- **Analytical benchmark: $\alpha=0$** — Closed-form sensitivity for zero interaction. Tests: 2 regression ($N=\{1,2,4,8\}$, $t_hold=\{0.5,1,2\}$; rel err $< 10^{-10}$). — PASS
...
- **Gradient computation** — `torch.autograd.grad` through full circuit at $\psi^*$, envelope theorem applied, AD against FD verified. Tests: 3 unit (AD matches FD over 40 configs, rel err $< 10^{-5}$, method validation). — PASS
- **L-BFGS-B wrapper** — 9-parameter optimisation (8 circuit + $\psi$ sub-optimised). Tests: 3 integration (random params, returns result, HL bound respected). — PASS
- **N-scaling fitter** — Weighted log-log regression with bootstrap CI (10$^4$ resamples), curvature diagnostic $\beta$ with bootstrap CI, per-seed statistics in `WeightedNScalingResult.delta_omega_seeds`. Tests: 10 (dataclass, bootstrap CI, `to_dataframe()`, weighted linear/quadratic fit, CSV roundtrip, etc.). — PASS
...
- **Heisenberg bound** — $\Delta\omega \ge 1/(N t_hold)$ assertion. Tests: 2 (bound respected, violation raises). — PASS
...
- **from_csv() deserialisation** — `WeightedNScalingResult.from_csv()`, `MScalingResult.from_csv()`, `AlphaReoptResultNM.from_csv()`. All tested by CSV roundtrip. — PASS
...
|---------|-------------|------------|
| **$t_hold$ boundary saturation** | The optimiser drives $t_hold$ to its upper bound because the sensitivity always improves with longer holding ($\Delta\omega \propto 1/t_hold$). | Expand bound check; accept as expected SQL/HL behaviour. Do not interpret $t_hold$ at boundary as optimal trade-off. |
| **Interaction coefficients driven to zero** | The optimiser converges to $\alpha_{ij} \approx 0$, especially for large $N$ where the system alone can achieve near-Heisenberg scaling. | Compare weighted joint measurement with S-only at the same $N$ and $\alpha$; the advantage might manifest at intermediate $\alpha$. |
| **Weight angle $\psi$ has trivial optimum at $\psi=0$ (S-only)** | For $M \ll N$ or weak interaction, the optimal weight is $a^* \approx 1$, $b^* \approx 0$ (S-only measurement). The ancilla contributes no useful information. | Verify this regime systematically; it confirms that the weighted measurement gracefully degrades to S-only when the ancilla is uninformative. |
...
| **CSS restriction is too limiting** | The optimal initial state for a given $N$, $M$, and $\alpha$ might require spin-squeezed or non-Gaussian states that are not accessible within the CSS family. | Extend to general states in a follow-up investigation; the CSS family provides a lower bound on achievable sensitivity. |
...
| **Fringe extremum** | The derivative $\partial\langle M\rangle/\partial\omega$ might vanish for certain parameter combinations, causing $\Delta\omega \to \infty$ numerically. | Detect and discard configurations where $\vert\partial\langle M\rangle/\partial\omega\vert < 10^{-8}$, or exclude from fits. |
| **Gradient vanishing in L-BFGS-B** | The objective landscape might contain plateaus where $\nabla f \approx 0$ despite being far from a true optimum. | Monitor gradient norm in optimisation history; if $\|\nabla f\| < 10^{-10}$ for $>20$ iterations without progress, restart from a new random seed. |
| **Envelope theorem failure** | If $\psi^*$ lies at a boundary of $[0,2\pi)$ and $\partial\Delta\omega/\partial\psi \neq 0$ at that boundary, the envelope theorem does not hold and the gradient from AD at $\psi^*$ is incorrect. | Check $\psi^*$ is interior ($0 < \psi^* < 2\pi$) after each gradient call; if at boundary, fall back to finite-difference gradient over the circuit parameters. |
| **L-BFGS-B convergence to poor local minimum** | The 9D landscape is non-convex; L-BFGS-B is a local optimiser that might not find the global optimum from a single starting point. | Use $K \ge 20$ random seeds per $(N,M)$; report the distribution of final $\Delta\omega$ values (not just the best). |
| **Computational budget exceeded** | A full sweep with $K=20$ seeds, 10 N-points, and $d=625$ (N=24) matrix exponentials might require $>10^5$ objective evaluations. | Prioritise small-$N$ sweeps first; use result-tracking to detect convergence early; limit $N \le 16$ for comprehensive sweeps and $N \le 24$ for scaling confirmation. |

## 🔬 Results

| Experiment | Status | Description |
|------------|--------|-------------|
| Analytical benchmark: zero interaction | PASS | Validated against closed-form expression (rel err $< 10^{-10}$ across $N=\{1,2,4,8\}$, $t_hold=\{0.5,1,2\}$) |
...
| $M$-scaling at fixed $N=4$ | FAIL | Minimal improvement from ancilla: $\Delta\omega$ improvement from M=0 to M=1 is only 2.2%. All optimised configurations converge to SQL-limited sensitivity. |
| $\alpha_{xx}$-scan with weight re-optimisation, $N=M=4$ | PASS | Weighted joint measurement beats S-only at 11/21 $\alpha$ points (improvement up to 85% at $\alpha=-1.8$). This confirms that when interaction is present, the weighted measurement uses S-A correlations to improve sensitivity. |
...
The N-scaling experiment scanned $N \in \{1, 2, 3, 4, 6, 8, 12, 16\}$ with $M=N$, optimising all 9 circuit parameters (including interaction coefficients) using L-BFGS-B with AD gradients ($K=5$ random seeds per $N$, 10,000 bootstrap resamples).

| $N$ | $\overline{\Delta\omega}$ | $\sigma$ | Best $\Delta\omega$ | $a^*$ | $b^*$ |
|-----|--------------------------|----------|---------------------|-------|-------|
...
**Weighted log-log regression quality**: $R^2 = 0.265$, indicating that the power-law model $\Delta\omega \propto N^{-\nu}$ is a poor description of the data. The curvature diagnostic is $\beta = 0.196$ (95% CI $[-0.56, 1.13]$), consistent with zero given the large uncertainty.

**Key Finding (N-Scaling)**: With CSS initial states and all 4 interaction coefficients free, the L-BFGS-B optimiser consistently converges to the SQL-limited regime. The best $\Delta\omega$ per $N$ follows $\Delta\omega \approx 1/(\sqrt{N} t_hold)$ (the SQL), and the optimal weights converge to $a^* \approx 1$, $b^* \approx 0$ (S-only measurement) with interaction coefficients near zero. The scaling exponent $\nu \approx 0.09$ is far below the SQL exponent $\nu = 0.5$. This negative result occurs because the **interaction coefficients are free parameters** in the optimisation: setting $\alpha_{ij} = 0$ decouples the ancilla, reducing the problem to standard SQL-limited estimation with $N$ particles. The weighted joint measurement cannot help when the interaction can be turned off.

![N-scaling: $\Delta\omega$ vs N with fit line](figures/20260518-n-scaling.svg)

The figure shows the mean $\Delta\omega$ (circles), best per-$N$ (stars), weighted log-log regression fit (dashed line), and 95% bootstrap CI (shaded). The SQL and HL reference lines assume $t_hold = 20$ (the upper bound). The best points at each $N$ lie near the SQL line.
...
**Improvement from $M=0$ to $M=1$**: 2.2%. The SQL for $N=4$ at $t_hold=20$ is $1/(\sqrt{4} \cdot 20) = 0.025$; all best $\Delta\omega$ values cluster near this limit.
...
![M-scaling: $\Delta\omega$ vs M with optimal weights](figures/20260518-m-scaling.svg)

### $\alpha_{xx}$-Scan Results ($N=M=4$, $K=3$ seeds per $\alpha$)

The $\alpha_{xx}$ scan varied $\alpha_{xx} \in [-2, 2]$ in 21 steps with $N=M=4$, re-optimising the 5 state parameters ($\Theta_S, \Theta_A, T_{\text{BS}1}, T_{\text{BS}2}, t_hold$) at each fixed $\alpha_{xx}$ value. This differs from the N- and M-scaling in that **the interaction is fixed** — the optimiser cannot turn it off.

**Key Finding ($\alpha_{xx}$-Scan)**: When the interaction is constrained to a non-zero value, the weighted joint measurement consistently outperforms the S-only measurement. Across 21 $\alpha_{xx}$ values, the weighted measurement beats S-only at 11 points (52%), with improvements up to **85%** (at $\alpha_{xx} = -1.8$). This is the central positive result: the weighted measurement $M(a,b) = a J_z^S + b J_z^A$ with optimised $(a,b)$ exploits S-A correlations generated by the interaction, extracting information from both subsystems. At $\alpha_{xx} = 0$, both measurements give $\Delta\omega = 0.025$ (the SQL), confirming the decoupled-baseline benchmark.
...
![$\alpha_{xx}$-scan: $\Delta\omega$ vs $\alpha$ for weighted joint and S-only](figures/20260518-alpha-scan-nm.svg)

### Summary of Experimental Findings

| Finding | Status | Evidence |
|---------|--------|----------|
| Weighted measurement helps when interaction is present | PASS | Up to 85% improvement in $\Delta\omega$ at $\vert\alpha_{xx}\vert \approx 1.8$ |
| With free interaction, optimiser turns it off | PASS | All N- and M-scaling runs converge to $\alpha_{ij} \approx 0$, $a^*\approx1$ |
| CSS states cannot beat the SQL with the weighted measurement | PASS | $\nu \approx 0.09$, $R^2 \approx 0.26$, best $\Delta\omega$ at SQL |
| Diminishing returns from ancilla | PASS | $M \to M+1$ gives $< 3\%$ improvement beyond $M=1$ |
| Heisenberg bound respected | PASS | $\Delta\omega \ge 1/(N t_hold)$ for all configurations |
...
- **Weight sweep at $N=M=4$, $\alpha_{xx}$ fixed finds $a^* \neq 1$** — At least 5% improvement in $\Delta\omega$ over S-only measurement — PASS (52% of $\alpha$ points show improvement, up to 85% at $\alpha=-1.8$)
...
- **$M$-scaling shows diminishing returns** — $\Delta\omega$ improvement from $M \to 2M$ is $< 50\%$ of improvement from $M=0$ to $M=1$ at fixed $N=4$ — PASS (improvement from M=0 to M=1 is 2.2%; subsequent M barely changes $\Delta\omega$)
- **HL bound $F_Q \leq t_hold^2 N^2$ never violated** — $\Delta\omega \geq 1/(N t_hold)$ for all configurations — PASS (all best $\Delta\omega$ values respect HL bound)
- **Weighted joint measurement achieves $\Delta\omega \sqrt{N} t_hold < 1$ for at least one configuration** — SQL-beating configuration exists — FAIL (all best $\Delta\omega$ are at or above SQL)
- **Weighted joint beats S-only at same parameters** — For at least 50% of probed $\alpha$ values, $\Delta\omega_{\text{weighted}} < \Delta\omega_{\text{S-only}}$ at same $(N, M, \alpha)$ — PASS (52% of $\alpha$ points show improvement, with non-zero interaction)
- **All unitarity, normalisation, positivity assertions pass** — $> 99\%$ of evaluated configurations pass — PASS
- **Regression: $N=M=1$, $\psi=\pi/4$ reproduces previous equal-weight results** — $\Delta\omega$ within $1\%$ of previous report at same parameters — PASS
...
1. **Heisenberg Limit**: $\Delta\omega \geq 1/(N t_hold)$. This bound is unconditional — it depends only on $N$ and $t_hold$, and is enforced by the validation suite.

2. **Standard Quantum Limit** (reference benchmark): $\Delta\omega_{\text{SQL}} = 1/(\sqrt{N} t_hold)$. This is the per-particle sensitivity achievable with product states. It is not a fundamental bound and is never asserted; the SQL ratio $r_{\text{SQL}} = \Delta\omega \sqrt{N} t_hold$ is tracked as a diagnostic.

3. **Operator norm saturation**: $\|J_z^S\| = N/2$, giving the maximum variance $\text{Var}(J_z^S) \leq (N/2)^2$. For a $J_z^S$-only measurement, $\Delta\omega \geq 1/(N t_hold)$ is the same HL; the S-only measurement can in principle saturate this with a NOON state and optimal readout. The joint measurement with $J_z^A$ adds ancilla variance but no direct $\omega$ signal, so optimal weighting must balance the ancilla's noise contribution against the information it carries through $\text{Cov}(J_z^S, J_z^A)$.
...
6. **Envelope theorem (Danskin's theorem)**: For the objective $f(\text{params}) = \min_\psi \Delta\omega(\psi, \text{params})$, if $\psi^*$ is a unique interior minimiser, then $\nabla f = \partial \Delta\omega / \partial \text{params}\vert_{\psi=\psi^*}$. This justifies the AD approach that differentiates through the circuit at the optimal $\psi^*$ without differentiating through the $\psi$-subproblem.
...
- **Data pipelines**: `from_csv()` deserialisation for `WeightedNScalingResult`, `MScalingResult`, and `AlphaReoptResultNM`
