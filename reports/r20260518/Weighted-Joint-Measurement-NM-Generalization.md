...
4. **The QFI bound remains $\Delta\omega \ge 1/(N t_hold)$**, set by the maximal eigenvalue $J_S = N/2$ of the generator $J_z^S$. The ancilla and the weighted measurement cannot surpass this Heisenberg limit, but can saturate it in the ideal interaction regime.
...
where $H_{\text{int}} = \alpha_{xx} J_x^S \otimes J_x^A + \alpha_{xz} J_x^S \otimes J_z^A + \alpha_{zx} J_z^S \otimes J_x^A + \alpha_{zz} J_z^S \otimes J_z^A.$ Crucially, **only the system S accumulates phase from $\omega$**; the ancilla experiences no $\omega$-dependent evolution. The hold unitary is $U_{\text{hold}}(t_hold) = \exp(-i t_hold H_{\text{hold}})$, computed via `torch.linalg.matrix_exp` acting on the $d$-dimensional space (the PyTorch implementation enables automatic differentiation through this operation). Third, the second beam splitter is applied. The complete evolution is
...
This is the **Heisenberg limit** for $N$ particles. The joint measurement with optimised weights respects this bound. The SQL ratio $r_{\text{SQL}} = \Delta\omega \sqrt{N} t_hold$ is tracked as a diagnostic benchmark: $r_{\text{SQL}} < 1$ indicates beating the SQL. It is never asserted.
...
4. **Hold unitary** — Compute $U_{\text{hold}}(t_hold) = \exp(-i t_hold H_{\text{hold}})$ via `torch.linalg.matrix_exp` on the full $d \times d$ matrix. $H_{\text{hold}}$ includes the $\omega$-dependent term $\omega J_z^S$ and the interaction $H_{\text{int}}$.

5. **Forward simulation function** — Define a differentiable PyTorch function that takes the 9 circuit parameters ($\Theta_S, \Theta_A, T_{\text{BS}1}, T_{\text{BS}2}, t_hold, \alpha_{xx}, \alpha_{xz}, \alpha_{zx}, \alpha_{zz}$), computes $\vert\Psi_{\text{final}}\rangle$ at $\omega$ and $\vert\Psi_{\text{final}}^{\pm}\rangle$ at $\omega \pm \delta$ (for the derivative), and returns the six moments ($\langle J_z^S\rangle, \langle J_z^A\rangle, \text{Var}(J_z^S), \text{Var}(J_z^A), \text{Cov}(J_z^S, J_z^A)$) and their $\omega$-derivatives computed via central finite differences with step $\delta = 10^{-6}$.
...
- **Hold unitary (torch)** — $\exp(-i t_hold [\omega J_z^S + H_{\text{int}}])$ via `torch.linalg.matrix_exp`. Tests: 4 integration (norm preservation, identity, scipy consistency). — PASS
...
- **Gradient computation** — `torch.autograd.grad` through full circuit at $\psi^*$, envelope theorem applied, AD vs FD verified. Tests: 3 unit (AD matches FD over 40 configs, rel err $< 10^{-5}$, method validation). — PASS
...
- **from_csv() deserialisation** — `WeightedNScalingResult.from_csv()`, `MScalingResult.from_csv()`, `AlphaReoptResultNM.from_csv()`. All tested via CSV roundtrip. — PASS
...
| **Fringe extremum** | The derivative $\partial\langle M\rangle/\partial\omega$ may vanish for certain parameter combinations, causing $\Delta\omega \to \infty$ numerically. | Detect and discard configurations where $\vert\partial\langle M\rangle/\partial\omega\vert < 10^{-8}$, or exclude from fits. |
...
| **L-BFGS-B convergence to poor local minimum** | The 9D landscape is non-convex; L-BFGS-B is a local optimiser that may not find the global optimum from a single starting point. | Use $K \ge 20$ random seeds per $(N,M)$; report the distribution of final $\Delta\omega$ values (not just the best). |
