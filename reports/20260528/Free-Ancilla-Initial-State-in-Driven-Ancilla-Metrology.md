...
For a system--ancilla pair of single-particle two-mode bosonic systems where the system S couples to the unknown phase $\theta$ via $H_S = \theta J_z^S$, the ancilla A is driven during the holding period by a controllable local Hamiltonian $H_A = a_x J_x^A + a_y J_y^A + a_z J_z^A$, and the system--ancilla interaction is the Ising-type $H_{\text{int}} = a_{zz} J_z^S \otimes J_z^A$, the sensitivity $\Delta\theta$ (error-propagation uncertainty in estimating $\theta$ via a $J_z^S$ measurement on the system) is investigated as a function of the ancilla initial state. The holding time is fixed at $T_H = 10$ for all experiments, giving an SQL reference of $\Delta\theta_{\text{SQL}} = 1/T_H = 0.1$.

The central question: does freeing the ancilla initial state from the fixed $|1,0\rangle$ to an arbitrary pure qubit state $|\psi_A\rangle = \alpha|1,0\rangle + \beta|0,1\rangle$ enable SQL violation ($\Delta\theta < 1/T_H$) that was previously unattainable?
...
**Null hypothesis**: No scenario achieves $\Delta\theta < 1/T_H$. The $J=1/2$ spectral radius bound on the system is insurmountable regardless of the ancilla initial state. Scenarios C and D are expected to recover exactly SQL (no violation) as secondary support.
...
The 20260519 report found SQL violation by making the ancilla drive itself $\theta$-dependent ($H_A = \theta \cdot \mathbf{a} \cdot \mathbf{J}^A$), creating parametric amplification. The present work tests a different mechanism: freeing the ancilla initial state while keeping the drive $\theta$-independent.

## 📊 Models Survey

| Model | Ancilla Init | Ancilla Drive ($H_A$) | Interaction ($a_{zz}$) | Expected min $\Delta\theta$ | Purpose |
|-------|-------------|----------------------|----------------------|---------------------------|---------|
...
4. **Random search in 6D** -- For each of the 5 $\theta$ values $\{0.1, 0.5, 1.0, 2.0, 5.0\}$, generate $N_{\text{samp}} = 2000$ random configurations with:
...
5. **Nelder--Mead refinement** -- For each $\theta$ value and each scenario, refine the best 30 random-search points via Nelder--Mead. Report best $\Delta\theta$, optimal parameters, and comparison across scenarios.

6. **2D slice $(\theta_A, a_{zz})$** -- A 201$\times$201 grid over $\theta_A \in [0, \pi]$ and $a_{zz} \in [-5, 5]$ with $a_x = a_y = a_z = 0$ (no ancilla drive) and $\phi_A = 0$. Run at each of the 5 $\theta$ values. This slice directly visualises whether a free ancilla state alone (without drive) can affect the sensitivity. Compare with the 20260527 $(a_z, a_{zz})$ slice which found 100% SQL-level points.

7. **Metadata recording** -- Every result entry records:
   - $\theta$, $T_H$, $\theta_A$, $\phi_A$, $\|\mathbf{a}\|$, $a_x, a_y, a_z, a_{zz}$
   - $\Delta\theta$, $\langle J_z^S \rangle$, $\text{Var}(J_z^S)$, $\partial\langle J_z^S\rangle/\partial\theta$
   - $\Delta\theta/\text{SQL}$ ratio, fringe-extremum flag
   - Experiment type (scenario A/B/C/D, or `slice_thetaA_azz`)
...
|-----------|-----------|-----------|-----------|-----------|---------------|---------|
| $\theta$ | $\{0.1, 0.5, 1.0, 2.0, 5.0\}$ | same | same | same | same | Phase rate dependence |
...
| $N_{\text{samp}}$ per $\theta$ | 500 | 2000 | 2000 | 2000 | N/A (grid) | Monte Carlo density |
| NM refinements per $\theta$ | 50 | 30 | 30 | 30 | N/A | Local optimisation |

### Validation

- **State normalisation**: $\|\vert\Psi_0\rangle\| = 1$ for all free-ancilla states, verified at construction.
- **Baseline recovery**: At Scenario A parameters ($\theta_A=0$, $a_x=a_y=a_z=a_{zz}=0$), $\Delta\theta = 0.1$ exactly.
...
- **Fringe-extremum exclusion**: Configurations with $|\partial\langle J_z^S\rangle/\partial\theta| < 10^{-12}$ flagged as $\Delta\theta = \infty$.
- **Slice consistency**: At $\theta_A=0$, the $(\theta_A, a_{zz})$ slice must reproduce the decoupled baseline $\Delta\theta = 0.1$ for all $a_{zz}$, matching the 20260527 $(a_z, a_{zz})$ slice result.
...
- **Cross-scenario comparison plot** -- Bar chart comparing best $\Delta\theta/\text{SQL}$ across the four scenarios for each $\theta$.
...
| Failure | Mitigation |
|---------|------------|
| **SQL bound holds for all scenarios (strong null)** -- No configuration of $(\theta_A, \phi_A, a_x, a_y, a_z, a_{zz})$ yields $\Delta\theta < 0.1$, confirming the spectral-radius bound is absolute regardless of ancilla initial state. | Accept the negative result. It confirms the $J=1/2$ bound is fundamental and not circumvented by the ancilla's initial state. Report the best achievable $\Delta\theta$ ratios across scenarios. |
...
| **Scenario C (no drive) recovers exact SQL at all $(\theta_A, a_{zz})$** -- Without ancilla drive, the free ancilla cannot generate useful S-A dynamics regardless of its initial state. The 2D slice shows a flat landscape at $\Delta\theta = 0.1$, analogous to the 20260527 $(a_z, a_{zz})$ slice. | This is expected: without $H_A$, the ancilla evolves only under $a_{zz} J_z^S \otimes J_z^A$ which commutes with $J_z^S$, so no sensitivity change. Report the flat landscape as confirming the drive's essential role. |
| **Fringe extremum dominates** -- For many parameter combinations, $\partial\langle J_z^S\rangle/\partial\theta$ vanishes, producing $\Delta\theta = \infty$. | Flag and exclude fringe-extremum points. Report fraction of finite points per scenario. The envelope is computed only over finite-$\Delta\theta$ configurations. |
...
With all 6 parameters freed $(\theta_A, \phi_A, a_x, a_y, a_z, a_{zz})$, the best sensitivity across all five $\theta$ values is $\Delta\theta = 0.1$ (ratio 0.9999999992 to 0.9999999958). The optimal ancilla initial states are never near the fixed $|1,0\rangle$ limit ($\theta_A^*$ ranges from $0.65$ to $2.99$ rad), yet the sensitivity remains exactly at the SQL. The optimiser explores a variety of $(a_x, a_y, a_z)$ drive configurations and $a_{zz}$ values, but always returns to the SQL floor.

| $\theta$ | Best $\Delta\theta$ | Ratio $\Delta\theta/\text{SQL}$ | $\theta_A^*$ (rad) | $\phi_A^*$ (rad) | $a_{zz}^*$ |
|----------|-------------------|-------------------------------|-------------------|-----------------|-----------|
...
With $a_{zz} = 0$ but free ancilla $(\theta_A, \phi_A)$ and free drive $(a_x, a_y, a_z)$, the best sensitivity is again $\Delta\theta = 0.1$ (ratio 0.9999999975 to 0.9999999992). Without the Ising interaction, the system and ancilla evolve independently under $H_S$ and $H_A$, so the measurement on the system is unaffected by the ancilla.
...
A $201 \times 201$ grid over $\theta_A \in [0, \pi]$ and $a_{zz} \in [-5, 5]$ with $a_x = a_y = a_z = 0$, $\phi_A = 0$, evaluated at five $\theta$ values. The sensitivity landscape is flat at $\Delta\theta = 0.1$ across the entire grid. Zero points out of 40,401 per $\theta$ fall below the SQL. The minimum ratio is $0.9999999971$ (at $\theta = 5.0$), consistent with finite-difference discretisation noise.

| $\theta$ | Min $\Delta\theta$ | Min Ratio | Points at SQL (%) | Points below SQL |
|----------|-------------------|-----------|-------------------|-----------------|
...
The bar chart below (Figure 1) shows the best $\Delta\theta/\text{SQL}$ ratio for each scenario at each $\theta$. All four scenarios converge to exactly $1.0$ (SQL), with the tiny variations ($\sim 4 \times 10^{-10}$) attributable to finite-difference numerical noise. No scenario outperforms any other.

![Cross-scenario comparison](figures/20260528-cross-scenario-comparison.svg)

**Figure 1**: Best $\Delta\theta/\text{SQL}$ ratio across four scenarios for each $\theta$ value. All bars sit exactly at the red SQL line.

The norm-envelope comparison (Figure 2) overlays the best-ratio-versus-norm curves for Scenarios A and B. Both envelopes are indistinguishable and sit at $\Delta\theta/\text{SQL} = 1$ across the entire parameter range.
...
| Scenario | Free Params | Best $\Delta\theta$ | $\min(\Delta\theta/\text{SQL})$ | SQL Violation? |
|----------|------------|-------------------|-------------------------------|---------------|
...
**Key Finding**: The $J=1/2$ spectral radius bound is absolute. Neither freeing the ancilla initial state, adding an ancilla drive, nor tuning the Ising interaction can produce $\Delta\theta < 1/T_H = 0.1$ for a single-particle ($N=1$) system.

## ✅ Success Criteria

- **Free-ancilla SQL violation (Scenario B)** -- $\exists\, (\theta_A, \phi_A, a_x, a_y, a_z, a_{zz}, \theta)$ with $\Delta\theta < 0.1 - 10^{-10}$, verified with $\mathcal{R} = \Delta\theta \times T_H < 1$. -- **FAIL**. The best $\Delta\theta$ in Scenario B is $0.1$ to machine precision (ratio $0.999999996$). The tiny deviations $\sim 4 \times 10^{-10}$ below the threshold are finite-difference numerical noise, not a physical SQL violation. The null hypothesis stands.
- **Scenario B vs A comparison** -- Best $\Delta\theta$ in Scenario B is strictly lower than best in Scenario A, confirming free ancilla provides measurable benefit even if SQL is not beaten. -- **FAIL**. Scenario B and A are indistinguishable: both converge to exactly $\Delta\theta = 0.1$ at every $\theta$ value. Freeing the ancilla initial state provides no measurable benefit over the fixed-ancilla baseline.
- **Scenario C (no drive) equivalence** -- Best $\Delta\theta$ in Scenario C equals SQL ($0.1$) for all $\theta$, confirming ancilla drive is necessary for any sensitivity modification. -- **PASS**. As predicted, without the ancilla drive, $[H, J_z^S] = 0$ and the sensitivity is identically SQL for all $\theta$, $\theta_A$, $\phi_A$, and $a_{zz}$.
- **Scenario D (no interaction) equivalence** -- Best $\Delta\theta$ in Scenario D equals SQL ($0.1$) for all $\theta$, confirming the Ising interaction is necessary for the free ancilla to affect $J_z^S$. -- **PASS**. Without $a_{zz}$, the system and ancilla evolve independently, and the sensitivity is exactly SQL.
- **2D slice flatness** -- $\min_{\theta_A, a_{zz}} \Delta\theta/\text{SQL} = 1.0$ for all $\theta$ when $H_A=0$, confirming the 20260527 $(a_z, a_{zz})$ result generalises to free $\theta_A$. -- **PASS**. The $(\theta_A, a_{zz})$ landscape is flat at $\Delta\theta = 0.1$ across all $40,401$ grid points. Zero points fall below SQL at any $\theta$.
- **Reproducibility** -- Scenario A reproduces the 20260518 baseline result (no SQL violation, best $\Delta\theta = 0.1$ to within $10^{-10}$). -- **PASS**. Scenario A yields $\Delta\theta = 0.1$ (ratio $0.999999996$ to $0.999999999$) at all $\theta$, confirming the 20260518 finding.
...
**Scenario B** (6 free parameters: $\theta_A, \phi_A, a_x, a_y, a_z, a_{zz}$) — the central experiment — converges to exactly $\Delta\theta = 0.1$ ($\Delta\theta \times T_H = 1$) at all five $\theta$ values. The optimal ancilla initial state varies widely across $\theta$ values (from $\theta_A^* = 0.65$ rad to $2.99$ rad), yet the sensitivity never falls below the SQL. This confirms that the $J=1/2$ spectral radius bound is absolute: no product initial state of the ancilla can unlock sub-SQL sensitivity.

**Freeing the ancilla initial state provides no measurable benefit** over the fixed $|1,0\rangle$ baseline. Scenarios A and B yield indistinguishable best sensitivities (both at $\Delta\theta = 0.1$). The optimiser in Scenario B does not converge to $\theta_A = 0$ (the fixed-ancilla limit), but instead finds a variety of $\theta_A$ values — yet none of them produce better sensitivity than the fixed-ancilla case. This indicates that the $J=1/2$ constraint is not merely a local optimum but a global bound.
...
**Comparison with 20260519**: The 20260519 report achieved SQL violation by making the ancilla drive itself $\theta$-dependent ($H_A = \theta \cdot \mathbf{a} \cdot \mathbf{J}^A$), creating parametric amplification analogous to a parametric amplifier. The present work shows that a $\theta$-independent drive — even when augmented with a free ancilla initial state — cannot achieve this effect. The $\theta$-dependence of the drive itself, not the ancilla initial state, is the essential ingredient for bypassing the SQL in this architecture.

**Implications**: The $J=1/2$ spectral radius bound on a single-particle system appears to be fundamental and cannot be circumvented by any product-state ancilla configuration (free initial state, drive, or Ising interaction). Beating the SQL in a two-qubit ancilla-assisted interferometer requires either (a) a $\theta$-modulated drive (as in 20260519), (b) entangled initial states, or (c) multi-particle ancillae with larger $J_A$.

**Open items**: (a) Could a general entangled initial state (Bell state, not product) unlock SQL violation? An entangled $|\Psi_0\rangle$ correlates the system and ancilla from the start, potentially increasing the effective generator. (b) Could the free ancilla be combined with $\theta$-modulated drive for additional enhancement beyond what 20260519 achieved alone? (c) Would a multi-particle ancilla ($J_A > 1/2$) with free initial state change the bound? A $J_A = 1$ ancilla has a larger Hilbert space and could host more complex dynamics during the holding period.
