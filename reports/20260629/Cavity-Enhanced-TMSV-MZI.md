...
1. **State preparation** — Reuse the TMSV state constructor $\sum_n c_n \vert n,n\rangle$ from `reports/20260625/heisenberg_limit_mzi_sq_oat.py` (`_make_two_mode_squeezed_vacuum`), which accepts a target total mean photon number $\langle N\rangle$ and truncation $M$ per mode.
