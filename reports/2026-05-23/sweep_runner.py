...
Usage:
    uv run python reports/2026-05-23/sweep_runner.py dual-sweep
    uv run python reports/2026-05-23/sweep_runner.py sonly-sweep
    uv run python reports/2026-05-23/sweep_runner.py all
"""
...
def run_dual_N(N_val: int, theta_arr: np.ndarray, force: bool = False) -> Path | None:
    """Run all theta for a single N, save to individual parquet."""
    ...
    thetas = np.zeros(len(theta_arr), dtype=float)
    Ns = np.full(len(theta_arr), N_val, dtype=int)
    protos = ["dual"] * len(theta_arr)
    a_xx = np.full(len(theta_arr), np.nan, dtype=float)
    a_xz = np.full(len(theta_arr), np.nan, dtype=float)
    a_zx = np.full(len(theta_arr), np.nan, dtype=float)
    a_zz = np.full(len(theta_arr), np.nan, dtype=float)
    delta_opts = np.full(len(theta_arr), np.inf, dtype=float)
    sqls = np.zeros(len(theta_arr), dtype=float)
    ratios = np.full(len(theta_arr), np.inf, dtype=float)
    exps = np.zeros(len(theta_arr), dtype=float)
    vars_ = np.zeros(len(theta_arr), dtype=float)
    d_exps = np.zeros(len(theta_arr), dtype=float)
    n_starts_arr = np.full(len(theta_arr), n_starts, dtype=int)
    n_conv_arr = np.zeros(len(theta_arr), dtype=int)
    grad_norm_arr = np.full(len(theta_arr), np.nan, dtype=float)

    total_theta = len(theta_arr)
    for i, theta_val in enumerate(theta_arr):
        ...
        delta_opts[i] = opt.delta_theta_opt
        sqls[i] = sql
        ratios[i] = ratio
...
def run_sonly_N(N_val: int, theta_arr: np.ndarray, force: bool = False) -> Path | None:
    """Run all theta for a single N in S-only sweep."""
...
def run_dual_sweep(force: bool = False) -> None:
    ...
    for N_val in N_vals:
        ...
        total_per_N = n_starts * len(theta_arr)
        est_sec = 0
        ...
        run_dual_N(N_val, theta_arr, force=force)
...
def run_sonly_sweep(force: bool = False) -> None:
    ...
    for N_val in N_vals:
        print(f"\n--- S-only N={N_val} ---")
        run_sonly_N(N_val, theta_arr, force=force)
