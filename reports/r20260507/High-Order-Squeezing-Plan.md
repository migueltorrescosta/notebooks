...
This plan describes using non-Gaussian squeezed states, generated by high-order spin-dependent forces (n >= 3), to outperform Gaussian squeezed states (n = 2) for phase estimation in Mach-Zehnder interferometers (MZIs) at fixed mean oscillator photon number, provided decoherence rates are below a critical threshold. A critical decoherence rate gamma_c exists such that:
- For gamma < gamma_c: F_Q(n=3 or 4) > F_Q(n=2) at fixed mean photon number
- For gamma > gamma_c: F_Q(n=2) >= F_Q(n=4) (Gaussian states are more robust)
...
**Open items**: (a) Decoherence sweeps to find gamma_c are ready but not yet run. (b) The adaptive Fock truncation (safety factor 10n) might be insufficient for n=4 at moderate mean photon number, which could affect QFI accuracy. Verify the post-hoc occupancy check (mean photon number <= 0.9 N_osc) during production sweeps.
