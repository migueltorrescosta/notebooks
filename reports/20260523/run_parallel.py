...
def _run_N_batch(
    N_val: int, omega_arr: np.ndarray, protocol: str
) -> FourParamSweepResult:
    """Run all omega values for a single N."""
    ...
    Ns = np.full(n_omega, N_val, dtype=int)
    protos = [protocol] * n_omega
    a_xx = np.full(n_omega, np.nan, dtype=float)
    a_xz = np.full(n_omega, np.nan, dtype=float)
    a_zx = np.full(n_omega, np.nan, dtype=float)
    a_zz = np.full(n_omega, np.nan, dtype=float)
    delta_opts = np.full(n_omega, np.inf, dtype=float)
    sqls = np.zeros(n_omega, dtype=float)
    ratios = np.full(n_omega, np.inf, dtype=float)
    exps = np.zeros(n_omega, dtype=float)
    vars_ = np.zeros(n_omega, dtype=float)
    d_exps = np.zeros(n_omega, dtype=float)
    n_starts_arr = np.full(n_omega, n_starts, dtype=int)
    n_conv_arr = np.zeros(n_omega, dtype=int)
    grad_norm_arr = np.full(n_omega, np.nan, dtype=float)
    ...
    for i, omega_val in enumerate(omega_arr):
        ...
        delta_opts[i] = opt_result.delta_omega_opt
        sqls[i] = sql
        ratios[i] = (
            opt_result.delta_omega_opt / sql
            if np.isfinite(opt_result.delta_omega_opt) and sql > 0
            else float("inf")
        )
        ...
        print(
            f"  [N={N_val:2d}, ω={omega_val:.1f}] "
            f"Δω={opt_result.delta_omega_opt:.6f}, "
            f"ratio={ratios[i]:.4f}, "
...
def run_dual_sweep_parallel(force: bool = False, workers: int = 8) -> None:
    ...
    print(
        f"  Total: {len(N_vals)} N × {len(omega_arr)} ω = {len(N_vals) * len(omega_arr)} points"
    )
    ...
    with ProcessPoolExecutor(max_workers=workers) as executor:
        for N_val in N_vals:
            future = executor.submit(_run_N_batch, N_val, omega_arr, "dual")
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                print(f"  [DONE] N={N_val}: {len(result.omega_values)} points")
    ...
    for N_val in sorted_N:
        ...
        all_omegas.extend(r.omega_values.tolist())
        ...
        all_delta.extend(r.delta_omega_opt.tolist())
    ...
    combined = FourParamSweepResult(
        omega_values=np.array(all_omegas, dtype=float),
        ...
        delta_omega_opt=np.array(all_delta, dtype=float),
    ...
    from local import plot_omega_scan, plot_ratio_heatmap
    ...
    fig_scan_p = (
        parquet_path("..").parent / "figures" / "20260523-dual-mzi-omega-scan-N5.svg"
    )
    plot_omega_scan(combined, fig_scan_p, N_fixed=5)
...
def main() -> None:
    ...
    if args.only is None:
        ...
        _run_single_sweep("S-only", force=args.force)
    elif args.only in ("n-scaling", "omega-dependence", "scaling-analysis"):
...
def _run_single_sweep(protocol: str, force: bool = False) -> None:
    ...
    print(f"  ω: {OMEGA_VALS}")
    print(
        f"  Points: {len(N_arr)} N × {len(omega_arr)} ω = {len(N_arr) * len(omega_arr)}"
    )

    total_points = len(N_arr) * len(omega_arr)
    point_count = 0
    all_results = []
    ...
    for N_val in N_arr:
        ...
        for omega_val in omega_arr:
            ...
            print(
                f"  [{protocol} N={N_val:2d}, ω={omega_val:.1f}] "
                f"Δω={opt.delta_omega_opt:.6f}, "
                f"ratio={ratio:.4f}, "
            ...
            all_results.append(
                {
                    ...
                    "delta_omega_opt": opt.delta_omega_opt,
                    "sql": opt.sql,
                    "ratio": ratio,
