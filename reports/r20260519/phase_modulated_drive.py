...
Usage:
    uv run python reports/r20260519/phase_modulated_drive.py --force
    uv run python reports/r20260519/phase_modulated_drive.py \
        --only phase-decoupled-baseline

This module is importable via
``importlib.import_module("reports.r20260519.phase_modulated_drive")``.
"""
...
def generate_phase_2d_slice_ax_azz(
    ...
    consolidate_raw_parquet(
        raw_data_dir=REPORTS_DIR / f"r{PHASE_DATE}" / "raw_data",
        glob_pattern=f"{PHASE_DATE}-phase-2d-slice-ax-azz-omega*.parquet",
        output_name=f"{PHASE_DATE}-phase-2d-slice-ax-azz.parquet",
        delete_shards=True,
    )
...
def generate_phase_2d_slice_ay_azz(
    ...
    consolidate_raw_parquet(
        raw_data_dir=REPORTS_DIR / f"r{PHASE_DATE}" / "raw_data",
        glob_pattern=f"{PHASE_DATE}-phase-2d-slice-ay-azz-omega*.parquet",
        output_name=f"{PHASE_DATE}-phase-2d-slice-ay-azz.parquet",
        delete_shards=True,
    )
...
def generate_phase_2d_slice_az_azz(
    ...
    consolidate_raw_parquet(
        raw_data_dir=REPORTS_DIR / f"r{PHASE_DATE}" / "raw_data",
        glob_pattern=f"{PHASE_DATE}-phase-2d-slice-az-azz-omega*.parquet",
        output_name=f"{PHASE_DATE}-phase-2d-slice-az-azz.parquet",
        delete_shards=True,
    )
...
def generate_phase_random_search(
    ...
    consolidate_raw_parquet(
        raw_data_dir=REPORTS_DIR / f"r{PHASE_DATE}" / "raw_data",
        glob_pattern=f"{PHASE_DATE}-phase-random-search-omega*.parquet",
        output_name=f"{PHASE_DATE}-phase-random-search.parquet",
        delete_shards=True,
    )
...
def generate_phase_cross_experiment_comparison(
    ...
    parquet_18 = (
        REPORTS_DIR / "r20260518" / "raw_data" / "20260518-drive-omega-scan.parquet"
    )
...
def generate_phase_supplementary_slices(force: bool = False) -> None:
    ...
    under ``reports/r20260519/figures/``.
...
def generate_article_figures(n_particles: int = 2) -> None:
    ...
    saved under ``reports/r20260519/figures/article-n2/``.
...
def main() -> None:
    ...
    (REPORTS_DIR / f"r{PHASE_DATE}" / "raw_data").mkdir(parents=True, exist_ok=True)
    (REPORTS_DIR / f"r{PHASE_DATE}" / "figures").mkdir(parents=True, exist_ok=True)
