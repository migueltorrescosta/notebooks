...
1. **SQL violation**: There exists a non-zero combination $(\alpha_{xx}, \alpha_{xz}, \alpha_{zx}, \alpha_{zz}) \neq (0,0,0,0)$ and a $\omega$ value such that $\Delta\omega < 1/T_H$, i.e., the sensitivity surpasses the $N=1$ SQL.
...
**Null hypothesis**: No combination of $(\alpha_{xx}, \alpha_{xz}, \alpha_{zx}, \alpha_{zz})$ with $H_A = \omega J_z^A$ can produce $\Delta\omega < 1/T_H$ when estimating $\omega$ via error-propagation on a $J_z^S$ measurement on the reduced system density matrix. The system's $J=1/2$ spectral radius bound remains insurmountable regardless of the interaction structure, because the reduced single-qubit state $\rho_S(\omega)$ is fundamentally QFI-bounded by $F_Q \leq T_H^2$.
...
4. **Hold unitary** — Compute $U_{\text{hold}}(T_H) = \exp(-i T_H H)$ via `scipy.linalg.expm`. The full Hamiltonian is $H = \omega(J_z^S + J_z^A) + H_{\text{int}}(\alpha)$ with Hermitian-symmetrisation $H \leftarrow \frac12 (H + H^\dagger)$ after construction.
...
6. **Sensitivity computation** — Compute $\langle J_z^S \rangle$ and $\text{Var}(J_z^S)$ from $\rho_S$. Compute $\partial\langle J_z^S\rangle / \partial\omega$ via central finite differences with $\delta = 10^{-6}$, re-evaluating the full circuit (including the trace) at $\omega \pm \delta$. The sensitivity is $\Delta\omega = \sqrt{\text{Var}(J_z^S)} / |\partial\langle J_z^S\rangle/\partial\omega|$.

7. **Optimisation via L-BFGS-B with multiple random starts** — For each fixed $\omega$ value, the objective is $f(\alpha_{xx}, \alpha_{xz}, \alpha_{zx}, \alpha_{zz}) = \Delta\omega(\alpha; \omega)$ to be minimised. Since $\Delta\omega$ may have many local minima (the 2026-05-20 report documented oscillatory behaviour in $\alpha_{xx}$), a multi-start L-BFGS-B strategy is used:
...
- **Derivative stability**: The central-difference derivative must produce $\Delta\omega$ values that are stable under changes to $\delta$ (e.g., $\delta \in [10^{-7}, 10^{-5}]$ produces the same $\Delta\omega$ to within $10^{-6}$ relative tolerance).
...
- **Holding unitary** — $\exp(-i T_H [\omega(J_z^S + J_z^A) + H_{\text{int}}(\alpha)])$ via `scipy.linalg.expm`.
...
| **L-BFGS-B converges to local minima** — the oscillatory $\Delta\omega(\alpha)$ landscape traps the optimizer in sub-optimal local minima, causing it to miss the global minimum | Use 100 random starts per $\omega$ value; monitor the distribution of final $\Delta\omega$ values; if the distribution is multi-modal, increase the number of random starts. Flag any run where $\Delta\omega > 10 \times \text{SQL}$ as a potential divergence (zero derivative). |
...
| **Fringe extremum** — some $\alpha$ values yield $\partial\langle J_z^S\rangle/\partial\omega \approx 0$, giving $\Delta\omega \to \infty$ (numerical blow-up) | Flag these configurations with $\Delta\omega = \infty$ in the CSV; exclude them from the $\omega$-scan. The L-BFGS-B optimizer will naturally avoid regions with zero gradient because the objective diverges. |
...
All experiments use a holding time $T_H = 10$, giving an SQL reference of $\Delta\omega_{\text{SQL}} = 1/T_H = 0.1$. Simulations were run via multi-start L-BFGS-B (100 random starts per $\omega$ value, bounded $\alpha_{ij} \in [-20, 20]$) and a decoupled baseline verification.
...
![Convergence metrics vs ω](figures/20260521-convergence.svg)

*Figure 2: Left panel: number of converged L-BFGS-B starts out of 100 per $\omega$ value. Right panel: $\Delta\omega / \text{SQL}$ ratio vs $\omega$, showing the best ratio of $0.690$ at $\omega = 3.8$.*
...
- **Optimal params recorded** — Full $\alpha^*(\omega)$ tuple and $\Delta\omega_{\text{opt}}(\omega)$ recorded for all 50 $\omega$ values in CSV — **PASS**: All 50 $\omega$ values recorded in `raw_data/20260521-omega-scan.csv` with full metadata.
- **CSV roundtrip** — All metadata fields survive serialisation/deserialisation roundtrip; fail-fast on missing columns — **PASS**: Verified via unit tests `test_bfgs_roundtrip_all_metadata`, `test_omega_scan_roundtrip_metadata`, `test_bfgs_from_csv_missing_columns`, and `test_omega_scan_from_csv_missing_columns`.
...
**Open items**: (a) Could a more sophisticated global optimisation method (Bayesian optimisation, basin-hopping, or evolutionary algorithms) find better minima that the 100-start L-BFGS-B approach missed? (b) Could a **different initial state** — such as an ancilla in a superposition of $J_z^A$ eigenstates — activate the $\alpha_{xz}$ and $\alpha_{zx}$ terms differently, potentially producing larger improvements? (c) Would a **joint measurement** $M = J_z^S + J_z^A$ (rather than tracing out the ancilla) extract the ancilla's $\omega$-dependent phase and produce sub-SQL sensitivity even with $H_A = \omega J_z^A$ or enable access to the full $F_Q = 4T_H^2$ bound of the two-qubit system? (d) Could **multiple ancilla particles** (spin-$J$ with $J > 1/2$) provide the spectral radius needed to overcome the $N=1$ system SQL by larger margins? (e) Would a **non-commuting ancilla drive** ($H_A = \omega(a_x J_x^A + a_y J_y^A + a_z J_z^A)$) combined with the general four-parameter interaction produce a different outcome from the 2026-05-19 result, where only Ising coupling was used?
