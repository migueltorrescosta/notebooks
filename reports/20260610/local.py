...
Usage:
    uv run python reports/20260610/local.py --force
    uv run python reports/20260610/local.py --only stage-1-slice
"""
...
def free_ancilla_modulated_2d_slice(
    ...
    if n_jobs is None or n_jobs == 1:
        ...
        with concurrent.futures.ProcessPoolExecutor(
            ...
            futures = {
                executor.submit(_modulated_slice_worker, args): args
...
def _modulated_slice_worker(args: tuple) -> tuple[int, np.ndarray]:
    """Worker for parallel (θ_A, a_zz) slice evaluation (module-level for pickle)."""
    (
        omega,
        theta_A_chunk,
        azz_vals,
