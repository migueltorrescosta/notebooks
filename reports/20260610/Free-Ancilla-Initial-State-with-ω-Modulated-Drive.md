...
**Null hypothesis**: Freeing the ancilla initial state provides no measurable benefit over the fixed $|1,0\rangle$ baseline at any $\omega$. The optimal parameters converge to $\theta_A^* \approx 0$ (the fixed-ancilla limit) or, if $\theta_A^*$ differs, the sensitivity is indistinguishable from the 20260519 baseline within numerical noise. The $J=1/2$ spectral radius bound on the $\omega$-modulated drive protocol is already saturated by the fixed-ancilla configuration.
...
|-------|-------------|---------------|----------------------|---------------------------|-----------|
| **A** (fixed-ancilla baseline) | Fixed $|1,0\rangle$ | $\omega$-modulated | Free | $\Delta\omega = 0.02036$ ($4.91\times$ SQL) | 20260519 |
