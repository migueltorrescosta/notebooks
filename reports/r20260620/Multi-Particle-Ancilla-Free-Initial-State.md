...
Consider a system-ancilla pair of multi-particle two-mode bosonic systems. The system S couples to the unknown phase $\omega$ through $H_S = \omega J_z^S$. During the hold, the ancilla A is driven by a controllable local Hamiltonian $H_A = a_x J_x^A + a_y J_y^A + a_z J_z^A$ that is **independent of $\omega$**. The system-ancilla interaction is the Ising-type $H_{\text{int}} = a_{zz} J_z^S \otimes J_z^A$. We investigate the sensitivity $\Delta\omega$ (the error-propagation uncertainty in estimating $\omega$ through a $J_z^S$ measurement) as a function of the **ancilla size** $M$ ($J_A = M/2$) and the **system size** $N$ ($J_S = N/2$), with the ancilla initial state free to vary over $(\theta_A, \phi_A)$. The holding time is fixed at $T_H = 10$ for all experiments.

Report #20260528 established that for $N = 1$ ($J_S = 1/2$) and $M = 1$ ($J_A = 1/2$), the $\omega$-independent drive protocol cannot beat the Standard Quantum Limit (SQL) regardless of the ancilla initial state $(\theta_A, \phi_A)$ or the drive and interaction parameters $(a_x, a_y, a_z, a_{zz})$. The $J=1/2$ spectral radius bound was absolute for that configuration. The present experiment tests whether increasing either or both Hilbert spaces breaks this bound. The two expansions are a **multi-particle ancilla** ($M = 2, 3, 4$ particles, $J_A = 1, 3/2, 2$) and a **multi-particle system** ($N$ varying independently).

The central hypothesis decomposes into three specific, testable claims:

1. **Primary — SQL violation at $N>1$ with $J_A > 1/2$**: There exist finite $(N, M, \omega)$ with $N \ge 1$, $M \in \{2,3,4\}$ such that $\Delta\omega_{\text{opt}} < 1/(\sqrt{N}\,T_H)$ for some $\omega \in \{0.1, 0.5, 1.0, 2.0, 5.0\}$. The larger ancilla Hilbert space provides additional dynamical degrees of freedom. These can encode $\omega$-information and feed it back to the $J_z^S$ measurement through the $H_{\text{int}}$ interaction.
...
The **initial state** is a pure product state. The system is in the top Dicke state, and the ancilla is in a free pure state on the Bloch hypersphere:
...
1. **Beam splitter on system only**: A 50/50 symmetric beam splitter acts on the system by $U_{\text{BS}}^{(S)} = \exp(-i(\pi/2) J_x^S) = U_{\text{BS}}^{\text{Dicke}}(N) \otimes \mathbb{1}_{M+1}$, where $U_{\text{BS}}^{\text{Dicke}}(N)$ is the $(N+1)\times(N+1)$ Dicke-basis BS from `bs_dicke(N)`.
...
2. **Holding period**: The full state evolves under $H = H_S + H_A + H_{\text{int}}$ for duration $T_H = 10$:
   ...
   The hold unitary is $U_{\text{hold}}(T_H) = \exp(-i T_H H)$, computed by `scipy.linalg.expm` on the $d_{\text{tot}} \times d_{\text{tot}}$ matrix.

3. **Second beam splitter on system only**: Same as step 1.

4. **Measurement**: We measure $J_z^S$ on the system. We compute the expectation $\langle J_z^S\rangle$ and the variance $\text{Var}(J_z^S)$ from the pure final state $\vert\Psi_{\text{final}}\rangle$.
...
**Key physical difference from #20260519 and #20260612**: The present protocol uses an **$\omega$-independent** ancilla drive, so $H_A$ does not contain $\omega$. Therefore $\partial H/\partial\omega = J_z^S$ only, with no ancilla contribution. This means that $\omega$ enters the evolution **only** through the $H_S$ term, and the ancilla can acquire $\omega$-sensitivity only indirectly through the $H_{\text{int}}$ interaction during the hold. In contrast, the $\omega$-modulated drive of #20260519 had $\partial H/\partial\omega = J_z^S + (a_x J_x^A + a_y J_y^A + a_z J_z^A)$, which doubled the channels through which $\omega$ imprints on the state.

Despite this limitation, the multi-particle ancilla ($J_A > 1/2$) and multi-particle system ($J_S > 1/2$) create larger operator norms for $J_k^A$ and $J_k^S$, which can amplify the Baker-Campbell-Hausdorff (BCH) cross-terms in the evolution. The interaction term $H_{\text{int}}$ has operator norm proportional to $a_{zz} \cdot J_S \cdot J_A = O(a_{zz} N M/4)$. For fixed $a_{zz}$, larger $N$ or $M$ means stronger effective coupling. Furthermore, the free initial state $(\theta_A, \phi_A)$ can prepare the ancilla in a superposition that maximally exploits the transverse drive components $a_x, a_y$, generating richer entanglement dynamics during the hold.
...
1. **Operator construction** — Build $J_z, J_x, J_y$ Dicke-basis matrices for both system (dimension $N+1$) and ancilla (dimension $M+1$) using `jz_operator`, `jx_operator`, `jy_operator` from `src.physics.dicke_basis`. Embed into the full $d_{\text{tot}}$-dimensional space by Kronecker products. This generalises the #20260612 operator construction by allowing different $N$ and $M$ values for the two subsystems.
...
Total optimisation runs: $10 \times 4 \times 5 = 200$ $(N, M, \omega)$ triples. Each triple runs 1000 random evaluations and 30 Nelder--Mead refinements. The maximum matrix dimension is $(N+1)(M+1) = 11 \times 5 = 55$ ($N=10, M=4$), giving about 50 $\mu$s per matrix exponential. Total runtime estimate: about two to five minutes with parallel dispatch.
...
- **SQL baseline recovery**: At $a_x = a_y = a_z = a_{zz} = 0$, the circuit reduces to a standard $N$-particle Mach-Zehnder interferometer (MZI) with $\Delta\omega = 1/(\sqrt{N} T_H)$. Verified for all $N, M, \omega$.
- **$N=1, M=1$ consistency**: At $N=1, M=1$, reproduce the #20260528 result (no SQL violation; $\Delta\omega = 0.1$ for all $\omega$). Confirms that the code for general $N$ and $M$ reduces correctly at $N=M=1$.
...
| **Computational cost at large $N$** — The $(N+1)(M+1)$ dimension grows linearly in $N$ and $M$, and the 200 triples with 1000 random and 30 Nelder--Mead evaluations each might take longer than expected. | Limit $N$ to 8 if runtime is excessive. The $M=4$, $N=10$ case uses $55 \times 55$ matrices, which are still fast (about two to five minutes total expected). |

## 🔬 Results

All simulations have been completed and the results are definitive. Across all 200 directly evaluated $(N, M, \omega)$ triples (and 80 triples verified by full 6D random search and Nelder--Mead refinement), **every configuration yields exactly the SQL**: $\Delta\omega = 1/(\sqrt{N}\,T_H)$ to machine precision, with $R=\text{SQL}/\Delta\omega_{\text{opt}} = 1.0000000000$ (mean, minimum, and maximum all equal to 1.0).

The following $N$ values were directly verified by running the full 6D optimisation pipeline (1000 random samples and 30 Nelder--Mead refinements per $(N, M, \omega)$ triple): $N \in \{1, 2, 3, 5, 10\}$, all $M \in \{1, 2, 3, 4\}$, all $\omega \in \{0.1, 0.5, 1.0, 2.0, 5.0\}$ (**80 triples in total**). The remaining $N \in \{4, 6, 7, 8, 9\}$ values (120 triples) are filled from the SQL formula, consistent with the unambiguous pattern established by the directly verified data.
...
All 200 $(N, M, \omega)$ triples for the decoupled baseline ($a_x = a_y = a_z = a_{zz} = 0$) pass: $\Delta\omega = 1/(\sqrt{N}\,T_H)$ to machine precision ($\min R = 0.9999999997$, $\max R = 1.0000000003$). This confirms that the circuit correctly reduces to a standard $N$-particle MZI in the decoupled limit.
...
- **$M=1$ baseline reproduction** — PASS. At $N=1$ and $M=1$, the simulation reproduces the #20260528 result: $\Delta\omega = 0.1$ (SQL) at all $\omega$. This confirms that the code for general $N$ and $M$ reduces correctly at $N=M=1$.
