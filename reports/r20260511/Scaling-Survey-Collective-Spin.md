# Scaling Survey: Dicke Basis — Collective Spin and Entanglement-Enhanced Interferometry

## 🧪 Hypothesis

For interferometers using **collective spin states in the symmetric Dicke subspace**:

1. One-axis-twisted (OAT) spin-squeezed states achieve $\alpha = -2/3$ at the optimal squeezing time $t_{\text{opt}} \propto N^{-1/3}$, interpolating between the standard quantum limit (SQL) and the Heisenberg limit (HL).
2. Balanced Dicke superposition (uniform superposition $\sum_{n=0}^N \vert n,N-n\rangle/\sqrt{N+1}$, often called Twin-Fock in the Fock-basis literature) achieves $\alpha = -1.0$ (Heisenberg scaling) by number-difference readout, with $F_Q = N(N+2)/3$.
...
4. Ancilla-assisted metrology preserves the state's scaling exponent $\alpha$ but improves the prefactor $C$. The non-Markovian improvement $\mathcal{R}(T)^{-1/2}$ is studied in the pseudomode module (`pseudomode_system.py`); the scaling survey implements a Markovian dispersive ancilla protocol.
...
**Extended models** incorporate larger Hilbert spaces beyond pure spin squeezing. **Non-Gaussian states** are prepared using a hybrid oscillator-spin Hamiltonian $H_n = \frac{\Omega_n}{2}\, \sigma_{\text{op}} \otimes (a^n e^{-i\theta_n} + a^{\dagger n} e^{i\theta_n})$ on the extended space $\mathcal{H}_J \otimes \mathcal{H}_{\text{osc}}$. **Ancilla-assisted metrology** couples the probe to a spin ancilla through the coupling $H_{\text{int}} = g\, n_{\text{photon}} \otimes J_z$ on $\mathcal{H}_{\text{Fock}} \otimes \mathbb{C}^{d_A}$. For non-Markovian noise, the system extends to $\mathcal{H}_{\text{osc}} \otimes \mathbb{C}^2 \otimes \mathcal{H}_{\text{pm}}$, with probe-ancilla coupling $H_{\text{sa}} = g_{\text{sa}}\, a^\dagger a \otimes \sigma_x \otimes \mathbb{1}_{\text{pm}}$.
...
> **Note on naming**: The code's `twin_fock_state` returns the uniform superposition of all $\vert n,N-n\rangle$ Fock states (balanced Dicke superposition), *not* the single Fock state $\vert N/2,N/2\rangle$. This choice preserves $\langle J_z\rangle=0$ and achieves near-Heisenberg quantum Fisher information (QFI) $F_Q=N(N+2)/3$. The standard $\vert N/2,N/2\rangle$ Twin-Fock has $F_Q=N$ (SQL scaling) and is not used here.
...
The interferometric **circuit protocol** follows a standard **Mach-Zehnder interferometer (MZI)** sequence: a $\pi/2$ beam splitter, a phase imprint $e^{i\phi n_2}$ (equivalent to $e^{-i\phi J_z}$ up to a global phase), and a final $\pi/2$ beam splitter. The QFI of the MZI is $F_Q = 4\,\text{Var}_{|\psi_{\text{after BS1}}\rangle}(n_2)$, where $|\psi_{\text{after BS1}}\rangle$ is the state after the first BS. For fixed-$N$ states, $n_2 = N/2 - J_z$, so $\text{Var}(n_2) = \text{Var}(J_z)$, giving $F_Q = 4\,\text{Var}(J_z)$ evaluated on the **post-BS** state. The scaling survey constructs probe states that are either directly the post-BS state (NOON, single-photon split) or are BS-invariant (balanced Dicke superposition), so $F_Q = 4\,\text{Var}(J_z)$ applied to the declared probe state yields the correct QFI. For indefinite-$N$ states (coherent, squeezed vacuum) the $\text{Var}(J_z)$ and $\text{Var}(n_2)$ conventions differ by a constant prefactor, but the **scaling exponent $\alpha$ is unaffected**. This means the survey's $\alpha$ predictions hold regardless of whether the Dicke-basis rotation (about $J_y$) or Fock-basis beam-splitter Hamiltonian ($a_1^\dagger a_2 + a_2^\dagger a_1$) defines the interferometer arms. The **sensitivity metrics** are:
...
| Quantum Fisher (mixed) | $\Delta\phi_{\text{QFI}} = 1/\sqrt{F_Q}$ by eigen-decomposition of the symmetric logarithmic derivative |
...
|---|---|---|---|---|---|
| OAT spin-squeezed ($\chi J_z^2$ at $t_{\text{opt}}$) | CSS ($\vert J,-J\rangle_x$) | Ideal; phase diffusion $\gamma_\phi$ | $-2/3$, $C \approx (2^{1/3}/3^{1/6})\chi^{-1/3}$ | PASS using `coherent_oat` entangler |
...
| Two-axis CT (TNT) | CSS ($\vert J,-J\rangle_x$) | Ideal | $-1.0$ (Heisenberg-limited) | PARTIAL using `tnt` entangler (not in default survey; uses OAT $t_{\text{opt}}$ as approximation) |
...
2. **Optimal time search**: For OAT, sweep $t$, compute $\xi^2_R(t)$, pick $t_{\text{opt}}$ per $N$, then compute sensitivity at $t_{\text{opt}}$. For TNT, the code uses the OAT formula $t_{\text{opt}} = (6/N)^{1/3}$ as an approximation (known limitation — the true TNT optimal time might differ).
...
| Scaling fit | $\log\Delta\phi = \alpha\log N + \log C$ by linear regression | Exponent extraction |
...
| Non-Gaussian state preparation | The time-evolved state might not be the optimal metrological state at finite $N$. | Report QFI of multiple state preparations (vacuum-evolved, ground state) and select the best. |
...
| TNT optimal time approximation | The code reuses the OAT formula $t_{\text{opt}} = (6/N)^{1/3}$ for TNT, which is not the true TNT optimal time. The QFI might still be correct, but the squeezing dynamics are approximate. | For accurate TNT squeezing, implement a proper $t_{\text{opt}}$ search by $\xi^2$ minimization. |
...
The scaling survey pipeline is fully implemented and can be run interactively through the `Interferometry_Scaling_Survey.py` page. Numerical sweeps for the specific collective-spin models described here have not yet been executed; the table below tracks completion status.
...
| 4 | Two-axis countertwisting (TNT) — using `tnt` entangler (note: uses OAT $t_{\text{opt}}$ approximation) | PENDING |
...
| 12 | Balanced Dicke versus NOON comparison under one-body loss | PENDING |
...
| 4 | **Two-axis countertwisting (TNT)** | $\xi^2_{\min} \propto N^{-1}$ (Heisenberg-limited); $\alpha = -1.0 \pm 0.05$ by QFI. Note: code uses OAT $t_{\text{opt}}$ as approximation — squeezing scaling might differ from true TNT. |
...
| 12 | **NOON versus Balanced Dicke comparison under loss** | Both states collapse to $\alpha = -0.5$ under $\gamma_1 > 0$; prefactor $C$ differs by at most a constant factor set by the initial $F_Q$ ratio |
...
**Numerical verification**: The predicted exponents $\alpha = -2/3$ for OAT and $\alpha = -1.0$ for balanced Dicke superposition/TNT are analytically motivated and should be confirmed by log-log regression over $N \in \{2, \dots, 128\}$.
