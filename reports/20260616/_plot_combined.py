...
def plot_combined_comparison() -> None:
    ...
    save_path = REPORTS_DIR / REPORT_DATE / "figures" / f"{REPORT_DATE}-step2-vs-step3-ratio.svg"
    ...
    for omega_val, colour in zip(omega_vals, colours):
        ...
        ax.plot(
            sub2["N"][valid2], sub2["ratio"][valid2],
            "o-", color=colour, markersize=5, linewidth=1.2,
            label=rf"$J_A=1/2$, $\omega={omega_val:.1f}$",
        )
        ax.plot(
            sub3["N"][valid3], sub3["ratio"][valid3],
            "s--", color=colour, markersize=5, linewidth=1.2,
    ...
    for omega_val, colour in zip(omega_vals, colours):
        ...
        ax.loglog(
            sub2["N"][valid2], sub2["delta_omega_opt"][valid2],
            "o-", color=colour, markersize=5, linewidth=1.2,
            label=rf"$J_A=1/2$, $\omega={omega_val:.1f}$",
        )
        ax.loglog(
            sub3["N"][valid3], sub3["delta_omega_opt"][valid3],
            "s--", color=colour, markersize=5, linewidth=1.2,
        )

    N_range = np.linspace(1, max(max(df2["N"]), max(df3["N"])), 100)
...
def plot_scaling_exponents() -> None:
    ...
    try:
        df = pd.read_parquet(REPORTS_DIR / REPORT_DATE / "raw_data" / f"{REPORT_DATE}-scaling-analysis.parquet")
    except FileNotFoundError:
        print("[skip] No scaling analysis data")
        return

    save_path = REPORTS_DIR / REPORT_DATE / "figures" / f"{REPORT_DATE}-scaling-exponents.svg"
    save_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    for scan_type, marker, label in [
        ...
        ax.plot(
            sub["omega"], sub["alpha"],
            f"{marker}-", markersize=8, linewidth=1.5,
            label=label,
        )

    ax.axhline(y=0, color="gray", linestyle=":", alpha=0.5, label=r"$\alpha=0$ (flat)")
    ax.axhline(y=-0.5, color="gray", linestyle="--", alpha=0.5, label=r"SQL: $\alpha=-0.5$")
    ax.axhline(y=-1.0, color="gray", linestyle="-.", alpha=0.3, label=r"HL: $\alpha=-1.0$")
