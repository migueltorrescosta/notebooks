...
| Twin-Fock | Uniform superposition $\sum_{n=0}^N \vert n, N-n\rangle/\sqrt{N+1}$ (Heisenberg-like scaling $F_Q = N(N+2)/3 \sim N^2/3$ under $J_z$; **not** the standard $\vert N/2, N/2\rangle$ twin-Fock which gives $F_Q = N$) |
...
| Ideal MZI twin-Fock | Uniform $\sum\vert n,N-n\rangle/\sqrt{N+1}$ | None | $-1.0$ ($F_Q = N(N+2)/3$, Heisenberg scaling $\Delta\phi \approx \sqrt{3}/N$) | PASS |
