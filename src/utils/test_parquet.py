...
class TestConsolidateSingleSchema:
    def test_given_three_shards_then_merged_file_has_all_rows(
        ...
        n = consolidate_raw_parquet(tmp_path, "shard-*.parquet", "merged.parquet")
    ...
    def test_given_delete_shards_then_originals_removed(self, tmp_path: Path) -> None:
        for i in range(2):
    ...
    def test_given_no_delete_shards_then_originals_kept(self, tmp_path: Path) -> None:
    ...
    def test_given_no_matches_then_raises(self, tmp_path: Path) -> None:
        with pytest.raises(FileNotFoundError, match="No files matching"):
            consolidate_raw_parquet(tmp_path, "nonexistent-*.parquet", "out.parquet")

    def test_given_single_file_then_output_has_one_row(self, tmp_path: Path) -> None:
        _write_shard(tmp_path / "only.parquet", {"z": [42.0]})

        n = consolidate_raw_parquet(tmp_path, "only*.parquet", "single.parquet")
    ...
    def test_given_metadata_columns_preserved(self, tmp_path: Path) -> None:
        ...
        consolidate_raw_parquet(tmp_path, "o*.parquet", "all-omegas.parquet")
...
class TestConsolidateMultiSchema:
    def test_given_two_schemas_then_two_output_files(self, tmp_path: Path) -> None:
        ...
        n = consolidate_raw_parquet(tmp_path, "*.parquet", "merged.parquet")
