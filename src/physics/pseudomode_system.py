"""
Pseudomode Method for Non-Markovian Ancilla-Assisted Metrology.

Physical Model:
- Tripartite system: Oscillator (probe) x Spin (ancilla) x Pseudomode (Lorentzian bath)
- H_osc = 0 (working frame, no free evolution)
- H_sa = g_sa . (a^dagger a) x sigma_x x I_pm (dispersive ancilla coupling)
- H_sp = g_sp . (a + a^dagger) x I_spin x (b + b^dagger) (system-pseudomode)
- H_pm = omega_0 . I_osc x I_spin x (b^dagger b) (pseudomode free energy)
- L_pm = sqrt(lambda) . I_osc x I_spin x b (pseudomode damping)

During the decoherence step, H_sa is turned OFF:
  H_dec = H_sp + H_pm

Hilbert Space:
- H_total = H_osc x H_spin x H_pm
- Index ordering: idx = (n * 2 + s) * (K + 1) + k
  - n: oscillator Fock index (0..N), slowest
  - s: spin (0=down, 1=up), middle
  - k: pseudomode Fock index (0..K), fastest
- Total dimension: 2 * (N + 1) * (K + 1)
- Built via nested Kronecker products: op_full = np.kron(np.kron(osc_op, spin_op), pm_op)

Units:
- Dimensionless throughout (hbar = 1)
"""
