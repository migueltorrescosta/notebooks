...
class TestPlotDrive2DSliceHeatmap:
    def test_creates_svg_file(self, tmp_svg: Path) -> None:
        result = Drive2DSliceResult(
            ...
            delta_theta_grid=np.random.default_rng(42).uniform(0.05, 0.3, (10, 8)),
            theta_value=1.0,
            slice_type="ax",
            sql=0.1,
        )
...
class TestPlotDriveRandomSearchHistogram:
    def test_creates_svg_file(self, tmp_svg: Path) -> None:
        ...
        result = DriveRandomSearchResult(
            ...
            delta_theta_values=rng.uniform(0.05, 0.3, 50),
            best_params=(1.0, 0.5, -0.3, 0.8),
            best_delta_theta=0.06,
            theta_value=1.0,
            sql=0.1,
            T_H=10.0,
        )
...
class TestPlotDriveThetaScan:
    def test_creates_svg_file(self, tmp_svg: Path) -> None:
        result = DriveThetaScanResult(
            ...
            variance_Jz_per_theta=np.array([0.25, 0.24, 0.26]),
        )
        out = plot_drive_theta_scan(result, tmp_svg)
        assert out.exists()
        assert out.suffix == ".svg"
        assert out.stat().st_size > 0
    ...
    def test_empty_sql_raises_valueerror(self) -> None:
        result = DriveThetaScanResult(
            ...
            best_params_per_theta=[(0.0, 0.0, 0.0, 0.0)],
            best_delta_theta_per_theta=np.array([0.1]),
            sql_values=np.array([]),
        )
        with tempfile.NamedTemporaryFile(suffix=".svg", delete=False) as f:
            path = Path(f.name)
        try:
            with pytest.raises(ValueError, match="sql_values is empty"):
                plot_drive_theta_scan(result, path)
        finally:
            path.unlink(missing_ok=True)
...
class TestPlotDriveNmExpectationVariance:
    def test_creates_svg_file(self, tmp_svg: Path) -> None:
        ...
        out = plot_drive_nm_expectation_variance(theta, exp_jz, var_jz, tmp_svg)
        assert out.exists()
        assert out.suffix == ".svg"
        assert out.stat().st_size > 0
...
class TestPlotDriveCrossExperimentComparison:
    def test_creates_svg_file(self, tmp_svg: Path) -> None:
        ...
        out = plot_drive_cross_experiment_comparison(
            theta, delta_19, delta_18, sql, tmp_svg
        )
        assert out.exists()
        assert out.suffix == ".svg"
        assert out.stat().st_size > 0
...
class TestPlotDriveFractionBelowSQL:
    def test_creates_svg_file(self, tmp_svg: Path) -> None:
        ...
        out = plot_drive_fraction_below_sql(
            theta, frac_ax, frac_ay, frac_random, tmp_svg
        )
        assert out.exists()
        assert out.suffix == ".svg"
        assert out.stat().st_size > 0
