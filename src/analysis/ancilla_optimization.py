...
- Holding Hamiltonian:
    H_hold = θ (J_z^S ⊗ I + I ⊗ J_z^A) + H_int(α)
...
def build_hold_hamiltonian(
    ...
    H = theta * (ops["Jz_S"] + ops["Jz_A"]) + H_int
    # Ensure Hermiticity
    H = 0.5 * (H + H.conj().T)
    return H
...
def compute_expectation_and_variance(
    ...
    var_val = max(0.0, exp_sq - exp_val**2)
...
def compute_reduced_purity(psi: np.ndarray) -> float:
    ...
    # Clamp to the mathematically valid range [0.5, 1.0]
    return float(min(1.0, max(0.5, purity)))
