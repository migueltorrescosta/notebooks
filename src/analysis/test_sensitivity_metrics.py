...
class TestErrorPropagationSensitivity:
    ...
    def test_error_propagation_single_photon(self) -> None:
        ...
        assert "delta_phi_ep" in result, 'Expected "delta_phi_ep" in result'
        assert "phi_at_min" in result, 'Expected "phi_at_min" in result'
        assert "delta_phi_grid" in result, 'Expected "delta_phi_grid" in result'
...
class TestAllSensitivityMetrics:
    ...
    def test_single_photon_sensitivity(self) -> None:
        ...
        assert "delta_phi_ep" in result, 'Expected "delta_phi_ep" in result'
        assert "delta_phi_fc" in result, 'Expected "delta_phi_fc" in result'
        assert "delta_phi_fq" in result, 'Expected "delta_phi_fq" in result'
    ...
    def test_noon_state_sensitivity(self) -> None:
        ...
        assert np.isfinite(result["delta_phi_fc"]), (
            'Expected result["delta_phi_fc"] to be finite'
        )
        assert np.isfinite(result["delta_phi_fq"]), (
            'Expected result["delta_phi_fq"] to be finite'
        )
...
class TestSensitivityValidation:
    ...
    def test_validate_sensitivity_order(self) -> None:
        ...
        assert validate_sensitivity_order(0.9, 1.0, rtol=0.2), (
            "Condition failed: validate_sensitivity_order(0.9, 1.0, rtol=0.2)"
        )
    ...
    def test_compare_sensitivity_methods(self) -> None:
        ...
        assert "delta_phi_ep" in result, 'Expected "delta_phi_ep" in result'
        assert "ep_valid" in result, 'Expected "ep_valid" in result'
        assert "methods_agree" in result, 'Expected "methods_agree" in result'

    def test_methods_agree_large_sample_limit(self) -> None:
        ...
        assert result["delta_phi_ep"] > 0, 'Expected result["delta_phi_ep"] > 0'
        assert result["delta_phi_fq"] > 0, 'Expected result["delta_phi_fq"] > 0'
        assert result["delta_phi_bayes"] > 0, 'Expected result["delta_phi_bayes"] > 0'
...
class TestReproducibility:
    ...
    def test_same_seed_same_results(self) -> None:
        ...
        assert result1["delta_phi_bayes"] == result2["delta_phi_bayes"], (
            'Expected result1["delta_phi_bayes"] == result2["delta_phi_bayes"]'
        )
    ...
    def test_different_seed_different_results(self) -> None:
        ...
        assert np.isfinite(result1["delta_phi_bayes"]), (
            'Expected result1["delta_phi_bayes"] to be finite'
        )
        assert np.isfinite(result2["delta_phi_bayes"]), (
            'Expected result2["delta_phi_bayes"] to be finite'
        )
