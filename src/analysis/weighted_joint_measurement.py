...
- Each subsystem is a two-mode bosonic Fock space with fixed particle number,
  described by the symmetric Dicke subspace of dimension N+1 and M+1.
- Circuit: BS1 (independent on S and A) -> Hold (theta J_z^S + H_int(alpha)) -> BS2.
- Measurement: weighted M(a,b) = a*J_z^S + b*J_z^A with a^2 + b^2 = 1.
- Sensitivity: error propagation Delta_theta = sqrt(Var(M)) / |d<M>/d_theta|.

The weight angle phi is sub-optimised via golden-section search at each circuit
evaluation, enabling the envelope theorem for gradient computation.
