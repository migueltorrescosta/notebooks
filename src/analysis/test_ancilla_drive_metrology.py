...
class TestDrive2DSlice:
    ...
    def test_invalid_slice_type_raises(self) -> None:
        with pytest.raises(ValueError, match="slice_type"):
            drive_2d_slice(theta=1.0, slice_type="invalid")

    def test_slice_csv_roundtrip(self, tmp_path: Path) -> None:
        result = drive_2d_slice(theta=1.0, slice_type="ax", n_drive=3, n_azz=3)
...
class TestDriveRandomSearch:
    def test_random_search_returns_correct_length(self) -> None:
        result = drive_random_search(theta=1.0, n_samples=50, seed=42)
        assert result.samples.shape == (50, 4)
        assert len(result.delta_theta_values) == 50

    def test_random_search_reproducible(self) -> None:
        r1 = drive_random_search(theta=1.0, n_samples=50, seed=42)
        r2 = drive_random_search(theta=1.0, n_samples=50, seed=42)
...
class TestDriveValidation:
    ...
    def test_given_random_search_then_best_not_worse_than_sql(self) -> None:
        ...
        result = drive_random_search(theta=1.0, n_samples=200, seed=42)
