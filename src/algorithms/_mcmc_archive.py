"""
Archived Metropolis-Hastings MCMC implementations.

These classes are no longer part of production code. For future MCMC needs,
use ``emcee`` or ``scipy.stats.sampling`` directly rather than creating
wrapper abstractions.

This module is retained only for Jupyter notebook compatibility and as a
reference example.

.. deprecated::
    ``AbstractMetropolisHastings`` and ``GaussianMetropolisHastings`` are
    archived and should not be used in new code.
"""
...
class AbstractMetropolisHastings[T](ABC):
    ...
    .. deprecated::
        Use ``emcee`` or ``scipy.stats.sampling`` instead.
